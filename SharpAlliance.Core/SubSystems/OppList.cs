...
public class OppList
{
    ...
    {
        if (gubNumShouldBecomeHostileOrSayQuote == 0
            || !DialogControl.DialogueQueueIsEmpty()
            || gfInTalkPanel
        ...
        else
        {
            ...
            else
            {
                ...
                // trigger hater
                NPC.TriggerNPCWithIHateYouQuote(Globals.MercPtrs[ubSpeaker].ubProfile);
            }
        }
    }
...
//                    }
            ...
            {
                sDistVisible += 1 * NUM_SKILL_TRAITS(pSoldier, SkillTrait.NIGHTOPS);
            }
    ...
    public static void TurnOffTeamsMuzzleFlashes(TEAM ubTeam)
    {
        int ubLoop;
        SOLDIERTYPE? pSoldier;
...
//        }
    ...
    public static int ManLooksForMan(SOLDIERTYPE? pSoldier, SOLDIERTYPE? pOpponent, int ubCaller)
    {
       ...
       */
        ...
        if (sDistAway <= sDistVisible)
        {
            ...
            if (LOS.SoldierToSoldierLineOfSightTest(pSoldier, pOpponent, (int)sDistVisible, bAware))
            {
        ...
        */
        ...
        if (bSuccess == 0 && (pPersOL == SEEN_CURRENTLY))
        {
    ...
    public static void ManSeesMan(SOLDIERTYPE? pSoldier, SOLDIERTYPE? pOpponent, int sOppGridno, int bOppLevel, int ubCaller, int ubCaller2)
    {
                        ...
                        if (fNotAddedToList)
                        {
                            switch (pSoldier.ubProfile)
                            {
                                ...
                                case NPCID.ANGEL:
                                    ...
                                    else if ((Facts.CheckFact(FACT.ANGEL_LEFT_DEED, NPCID.ANGEL) == true) && (Facts.CheckFact(FACT.ANGEL_MENTIONED_DEED, NPCID.ANGEL) == false))
                                    {
                                        AIMain.CancelAIAction(pSoldier, 1);
        ...
        {
            if (gubBestToMakeSightingSize != BEST_SIGHTING_ARRAY_SIZE_INCOMBAT && gTacticalStatus.bBoxingState == BoxingStates.NOT_BOXING)
            {
                if (fNewOpponent)
                {
                    if (gTacticalStatus.uiFlags.HasFlag(TacticalEngineStatus.INCOMBAT))
    ...
    public static void OtherTeamsLookForMan(SOLDIERTYPE? pOpponent)
    {
        ...
        for (uiLoop = 0; uiLoop < guiNumMercSlots; uiLoop++)
        {
            ...
            // if this merc is active, in this sector, and well enough to look
            if (pSoldier != null && pSoldier.bLife >= OKLIFE && (pSoldier.ubBodyType != SoldierBodyTypes.LARVAE_MONSTER))
            {
                // if this merc is on the same team as the target soldier
                if (pSoldier.bTeam == pOpponent.bTeam)
                {
...
//                            }
    ...
    void RemoveManAsTarget(SOLDIERTYPE? pSoldier)
    {
        ...
        */
        ...
        this.ResetLastKnownLocs(pSoldier);
                ...
                if (!(pSoldier.usQuoteSaidFlags.HasFlag(SOLDIER_QUOTE.SAID_MULTIPLE_CREATURES)))
                {
                    ...
                    for (cnt = 0; cnt < guiNumMercSlots; cnt++)
                    {
                        ...
                        if (pTeamSoldier != null)
                        {
                            if (OK_ENEMY_MERC(pTeamSoldier))
                            {
                                if (pTeamSoldier.uiStatusFlags.HasFlag(SOLDIER.MONSTER)
                ...
                if (ubTeamToRadioTo != MILITIA_TEAM)
                {
                    if (!gbShowEnemies && (pOpponent.bLife >= OKLIFE))
                    {
                        ...
                        if (fContactSeen)
                        {
                            ...
                            if (pOpponent.ubBodyType == SoldierBodyTypes.BLOODCAT)
                            {
                                // 2 is for bloodcat
...
//            pDoorStatus = GetDoorStatus(usMapPos);
    ...
    void WriteQuantityAndAttachments(OBJECTTYPE? pObject, int yp)
    {
            ...
            || pObject.usAttachItem[3] > 0)
        {
            fAttachments = true;
            szAttach = wprintf("(");
            this.AppendAttachmentCode(pObject.usAttachItem[0], ref szAttach);
            this.AppendAttachmentCode(pObject.usAttachItem[1], ref szAttach);
            this.AppendAttachmentCode(pObject.usAttachItem[2], ref szAttach);
            this.AppendAttachmentCode(pObject.usAttachItem[3], ref szAttach);
            wcscat(szAttach, " )");
        }
    ...
    void DebugSoldierPage4()
    {
        ...
        //        if (FindSoldierFromMouse(out usSoldierIndex, out uiMercFlags))
        {
            ...
            this.WriteQuantityAndAttachments(pSoldier.inv[InventorySlot.HELMETPOS], LINE_HEIGHT * ubLine);
            ubLine++;
            ...
            this.WriteQuantityAndAttachments(pSoldier.inv[InventorySlot.VESTPOS], LINE_HEIGHT * ubLine);
            ubLine++;
            ...
            this.WriteQuantityAndAttachments(pSoldier.inv[InventorySlot.LEGPOS], LINE_HEIGHT * ubLine);
            ubLine++;
            ...
            this.WriteQuantityAndAttachments(pSoldier.inv[InventorySlot.HEAD1POS], LINE_HEIGHT * ubLine);
            ubLine++;
            ...
            this.WriteQuantityAndAttachments(pSoldier.inv[InventorySlot.HEAD2POS], LINE_HEIGHT * ubLine);
            ubLine++;
            ...
            this.WriteQuantityAndAttachments(pSoldier.inv[InventorySlot.HANDPOS], LINE_HEIGHT * ubLine);
            ubLine++;
            ...
            this.WriteQuantityAndAttachments(pSoldier.inv[InventorySlot.SECONDHANDPOS], LINE_HEIGHT * ubLine);
            ubLine++;
            ...
            this.WriteQuantityAndAttachments(pSoldier.inv[InventorySlot.BIGPOCK1POS], LINE_HEIGHT * ubLine);
            ubLine++;
            ...
            this.WriteQuantityAndAttachments(pSoldier.inv[InventorySlot.BIGPOCK2POS], LINE_HEIGHT * ubLine);
            ubLine++;
            ...
            this.WriteQuantityAndAttachments(pSoldier.inv[InventorySlot.BIGPOCK3POS], LINE_HEIGHT * ubLine);
            ubLine++;
            ...
            this.WriteQuantityAndAttachments(pSoldier.inv[InventorySlot.BIGPOCK4POS], LINE_HEIGHT * ubLine);
            ubLine++;
            ...
            this.WriteQuantityAndAttachments(pSoldier.inv[InventorySlot.SMALLPOCK1POS], LINE_HEIGHT * ubLine);
            ubLine++;
            ...
            this.WriteQuantityAndAttachments(pSoldier.inv[InventorySlot.SMALLPOCK2POS], LINE_HEIGHT * ubLine);
            ubLine++;
            ...
            this.WriteQuantityAndAttachments(pSoldier.inv[InventorySlot.SMALLPOCK3POS], LINE_HEIGHT * ubLine);
            ubLine++;
            ...
            this.WriteQuantityAndAttachments(pSoldier.inv[InventorySlot.SMALLPOCK4POS], LINE_HEIGHT * ubLine);
            ubLine++;
            ...
            this.WriteQuantityAndAttachments(pSoldier.inv[InventorySlot.SMALLPOCK5POS], LINE_HEIGHT * ubLine);
            ubLine++;
            ...
            this.WriteQuantityAndAttachments(pSoldier.inv[InventorySlot.SMALLPOCK6POS], LINE_HEIGHT * ubLine);
            ubLine++;
            ...
            this.WriteQuantityAndAttachments(pSoldier.inv[InventorySlot.SMALLPOCK7POS], LINE_HEIGHT * ubLine);
            ubLine++;
            ...
            this.WriteQuantityAndAttachments(pSoldier.inv[InventorySlot.SMALLPOCK8POS], LINE_HEIGHT * ubLine);
            ubLine++;
        }
        //        else
        {
    ...
    int MovementNoise(SOLDIERTYPE? pSoldier)
    {
        ...
        int ubMaxVolume, ubVolume, ubBandaged, ubEffLife = 0;
        int bInWater = 0;
        ...
        else if (HAS_SKILL_TRAIT(pSoldier, SkillTrait.STEALTHY))
        {
            iStealthSkill += 25 * NUM_SKILL_TRAITS(pSoldier, SkillTrait.STEALTHY);
        }
    ...
    int DoorOpeningNoise(SOLDIERTYPE? pSoldier)
    {
        ...
        if (this.MovementNoise(pSoldier) > 0)
        {
    ...
    void OurNoise(int ubNoiseMaker, int sGridNo, int bLevel, int ubTerrType, int ubVolume, NOISE ubNoiseType)
    {
        ...
        this.ProcessNoise(ubNoiseMaker, sGridNo, bLevel, ubTerrType, ubVolume, ubNoiseType);
        ...
        for (bTeam = 0; bTeam < (TEAM)MAXTEAMS; bTeam++)
        {
            ...
            // #endif
            {
                ...
                //                foreach(var bLoop in gTacticalStatus.Team[bTeam])
                {
                    ...
                    ubEffVolume = this.CalcEffVolume(pSoldier, sGridNo, bLevel, ubNoiseType, ubBaseVolume, bCheckTerrain, pSoldier.bOverTerrainType, ubSourceTerrType);
                    ...
                    if (ubEffVolume > 0)
                    {
                        ...
                        this.HearNoise(pSoldier, ubSource, sGridNo, bLevel, ubEffVolume, ubNoiseType, out bSeen);
...
//                            || !(pSoldier.ubMovementNoiseHeard & (1 << ubNoiseDir)))
                    ...
                    */
                    {
                        ...
                        if (bTellPlayer && ubHeardLoudestBy != Globals.NOBODY)
                        {
                            ...
                            this.TellPlayerAboutNoise(Globals.MercPtrs[ubHeardLoudestBy], ubNoiseMaker, sGridNo, bLevel, ubLoudestEffVolume, ubNoiseType, ubLoudestNoiseDir);
                ...
                if (gTacticalStatus.Team[bTeam].IsHuman && (ubSource < NOBODY))
                {
                    ...
                    if (bSeen ?? false)
                    {
                        ...
                        if (true /* bTeam == Net.pnum */)
                        {
                            ...
                            this.OurTeamRadiosRandomlyAbout(ubSource);
                        }
    ...
    void HearNoise(SOLDIERTYPE? pSoldier, int ubNoiseMaker, int sGridNo, int bLevel, int ubVolume, NOISE ubNoiseType, out bool? ubSeen)
    {
        ...
        if (PythSpacesAway(pSoldier.sGridNo, sGridNo) <= sDistVisible)
        {
            ...
            {
                if (LOS.SoldierTo3DLocationLineOfSightTest(pSoldier, sGridNo, bLevel, 0, sDistVisible, 1))
                {
...
//            Messages.ScreenMsg(MSG_FONT_YELLOW, MSG.INTERFACE, pNewNoiseStr[ubNoiseType], pSoldier.name, pNoiseVolStr[ubVolumeIndex], gzLateLocalizedString[7]);
    ...
    public static void VerifyAndDecayOpplist(SOLDIERTYPE pSoldier)
    {
        int uiLoop;
    ...
    public static void DecayIndividualOpplist(SOLDIERTYPE? pSoldier)
    {
        int uiLoop;
        int? pPersOL;           // pointer into soldier's opponent list
        SOLDIERTYPE? pOpponent;
    ...
    public static void NoticeUnseenAttacker(SOLDIERTYPE? pAttacker, SOLDIERTYPE? pDefender, int bReason)
    {
        ...
        if (fSeesAttacker)
        {
            ManSeesMan(pDefender, pAttacker, pAttacker.sGridNo, pAttacker.bLevel, NOTICEUNSEENATTACKERS, CALLER_UNKNOWN);
...
//        }
    ...
    void DecayWatchedLocs(TEAM bTeam)
    {
        ...
        for (cnt = gTacticalStatus.Team[bTeam].bFirstID; cnt <= gTacticalStatus.Team[bTeam].bLastID; cnt++)
        {
            ...
            for (cnt2 = 0; cnt2 < NUM_WATCHED_LOCS; cnt2++)
            {
                if (gsWatchedLoc[cnt, cnt2] != NOWHERE && this.WatchedLocLocationIsEmpty(gsWatchedLoc[cnt, cnt2], gbWatchedLocLevel[cnt, cnt2], bTeam))
                {
