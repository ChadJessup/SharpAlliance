...
public class OppList
{
    ...
    int TeamNoLongerSeesMan(TEAM ubTeam, SOLDIERTYPE? pOpponent, int ubExcludeID, int bIteration)
    {
        ...
        for (pMate = Globals.MercPtrs[bLoop]; bLoop <= gTacticalStatus.Team[ubTeam].bLastID; bLoop++, pMate++)
        {
            ...
            {
                return (0);     // that's all I need to know, get out of here
            }
        }
    ...
    void AllTeamsLookForAll(int ubAllowInterrupts)
    {
        ...
        if ((gTacticalStatus.uiFlags.HasFlag(LOADING_SAVED_GAME)))
        {
            return;
        }

        if (ubAllowInterrupts || !(gTacticalStatus.uiFlags.HasFlag(TacticalEngineStatus.INCOMBAT)))
        {
            gubBestToMakeSightingSize = BEST_SIGHTING_ARRAY_SIZE_ALL_TEAMS_LOOK_FOR_ALL;
            if (gfDelayResolvingBestSightingDueToDoor)
            {
    ...
    void ManLooksForOtherTeams(SOLDIERTYPE? pSoldier)
    {
                        ...
                        if (pSoldier.bTeam == CIV_TEAM)
                        {
                            ...
                            else if (pSoldier.ubCivilianGroup == KINGPIN_CIV_GROUP)
                            {
                                ...
                                else
                                {
                                    ...
                                    if (InARoom(pOpponent.sGridNo, out ubRoom) && IN_BROTHEL_GUARD_ROOM(ubRoom))
                                    {
                                        // unauthorized!
                                        MakeCivHostile(pSoldier, 2);
                                        if (!(gTacticalStatus.uiFlags.HasFlag(TacticalEngineStatus.INCOMBAT)))
        ...
        */
        ...
        void RadioSightings(SOLDIERTYPE? pSoldier, int ubAbout, int ubTeamToRadioTo)
        {
            SOLDIERTYPE? pOpponent;
            int iLoop;
        ...
        void ProcessNoise(int ubNoiseMaker, int sGridNo, int bLevel, int ubTerrType, int ubBaseVolume, int ubNoiseType)
        {
            ...
            for (bTeam = 0; bTeam < MAXTEAMS; bTeam++)
            {
                ...
                // #endif
                {
                    ...
                    if (gTacticalStatus.Team[bTeam].bHuman && (ubSource < Globals.NOBODY))
                    {
            ...
            void HearNoise(SOLDIERTYPE? pSoldier, int ubNoiseMaker, int sGridNo, int bLevel, int ubVolume,
                    int ubNoiseType, out int? ubSeen)
            {
                int sNoiseX, sNoiseY;
