...
public class InterfaceCursors
{
    ...
    bool DrawUICursor()
    {
        ...
        if (GetMouseMapPos(&usMapPos))
        {
            ...
            if (gUICursors[guiCurUICursor].uiFlags & UICURSOR.FREEFLOWING && !(gUICursors[guiCurUICursor].uiFlags & UICURSOR.DONTSHOW2NDLEVEL))
            {
            ...
            if (gUICursors[guiCurUICursor].uiFlags & UICURSOR.SHOWTILEAPDEPENDENT)
            {
            ...
            if (gUICursors[guiCurUICursor].uiFlags & UICURSOR.CENTERAPS)
            {
    ...
    bool HideUICursor()
    {
        ...
        if (gUICursors[guiCurUICursor].uiFlags & (UICURSOR.SHOWTILE | UICURSOR.SHOWTILEAPDEPENDENT))
        {
            RemoveAllTopmostsOfTypeRange(gusCurMousePos, TileDefines.FIRSTPOINTERS, TileDefines.FIRSTPOINTERS);
            RemoveAllOnRoofsOfTypeRange(gusCurMousePos, TileDefines.FIRSTPOINTERS, TileDefines.FIRSTPOINTERS);
        }
    ...
    void DrawSnappingCursor()
    {
        ...
        switch (guiCurUICursor)
        {
            ...
            case UICursorDefines.CONFIRM_MOVE_RUN_UICURSOR:
                ...
                {
                    AddUIElem(gusCurMousePos, TileDefines.GOODRUN1, 0, -WALL_HEIGHT - 8, &pNewUIElem);
                }
                else
                {
                    AddUIElem(gusCurMousePos, TileDefines.GOODRUN1, 0, 0, &pNewUIElem);
                }
            ...
            case UICursorDefines.CONFIRM_MOVE_WALK_UICURSOR:
                if (gsInterfaceLevel > 0)
                {
                    AddUIElem(gusCurMousePos, TileDefines.GOODWALK1, 0, -WALL_HEIGHT - 8, &pNewUIElem);
                }
                else
                {
                    AddUIElem(gusCurMousePos, TileDefines.GOODWALK1, 0, 0, &pNewUIElem);
                }
                pNewUIElem.ubShadeLevel = Shading.DEFAULT_SHADE_LEVEL;
                pNewUIElem.ubNaturalShadeLevel = Shading.DEFAULT_SHADE_LEVEL;
                break;
            ...
            case UICursorDefines.CONFIRM_MOVE_SWAT_UICURSOR:
                if (gsInterfaceLevel > 0)
                {
                    AddUIElem(gusCurMousePos, TileDefines.GOODSWAT1, 0, -WALL_HEIGHT - 8, &pNewUIElem);
                }
                else
                {
                    AddUIElem(gusCurMousePos, TileDefines.GOODSWAT1, 0, 0, &pNewUIElem);
                }
                pNewUIElem.ubShadeLevel = Shading.DEFAULT_SHADE_LEVEL;
                pNewUIElem.ubNaturalShadeLevel = Shading.DEFAULT_SHADE_LEVEL;
                break;
            ...
            case UICursorDefines.CONFIRM_MOVE_PRONE_UICURSOR:
                if (gsInterfaceLevel > 0)
                {
                    AddUIElem(gusCurMousePos, TileDefines.GOODPRONE1, 0, -WALL_HEIGHT - 8 - 6, &pNewUIElem);
                }
                else
                {
                    AddUIElem(gusCurMousePos, TileDefines.GOODPRONE1, 0, -6, &pNewUIElem);
                }
                pNewUIElem.ubShadeLevel = Shading.DEFAULT_SHADE_LEVEL;
                pNewUIElem.ubNaturalShadeLevel = Shading.DEFAULT_SHADE_LEVEL;
                break;
            ...
            case UICursorDefines.CONFIRM_MOVE_VEHICLE_UICURSOR:
                if (gsInterfaceLevel > 0)
                {
                    AddUIElem(gusCurMousePos, TileDefines.VEHICLEMOVE1, 0, -WALL_HEIGHT - 8, &pNewUIElem);
                }
                else
                {
                    AddUIElem(gusCurMousePos, TileDefines.VEHICLEMOVE1, 0, 0, &pNewUIElem);
                }
                pNewUIElem.ubShadeLevel = Shading.DEFAULT_SHADE_LEVEL;
                pNewUIElem.ubNaturalShadeLevel = Shading.DEFAULT_SHADE_LEVEL;
                break;
            ...
            case UICursorDefines.CANNOT_MOVE_UICURSOR:
                ...
                if (gsInterfaceLevel > 0)
                {
                    AddUIElem(gusCurMousePos, TileDefines.BADMARKER1, 0, -WALL_HEIGHT - 8, &pNewUIElem);
                    pNewUIElem.ubShadeLevel = Shading.DEFAULT_SHADE_LEVEL;
                    pNewUIElem.ubNaturalShadeLevel = Shading.DEFAULT_SHADE_LEVEL;
    ...
    {
        RemoveAllTopmostsOfTypeRange(gusCurMousePos, TileDefines.MOCKFLOOR, TileDefines.MOCKFLOOR);
        RemoveAllTopmostsOfTypeRange(gusCurMousePos, TileDefines.FIRSTPOINTERS, TileDefines.LASTPOINTERS);
        RemoveAllObjectsOfTypeRange(gusCurMousePos, TileDefines.FIRSTPOINTERS, TileDefines.LASTPOINTERS);
        RemoveAllOnRoofsOfTypeRange(gusCurMousePos, TileDefines.FIRSTPOINTERS, TileDefines.LASTPOINTERS);
        RemoveAllOnRoofsOfTypeRange(gusCurMousePos, TileDefines.MOCKFLOOR, TileDefines.MOCKFLOOR);
    }
    ...
    void HandleLooseCursorDraw()
    {
        ...
        if (gfLooseCursorOn)
        {
            AddUIElem(gsLooseCursorGridNo, TileDefines.FIRSTPOINTERS4, 0, 0, &pNewUIElem);
            pNewUIElem.ubShadeLevel = Shading.DEFAULT_SHADE_LEVEL;
            pNewUIElem.ubNaturalShadeLevel = Shading.DEFAULT_SHADE_LEVEL;
        }
    }
