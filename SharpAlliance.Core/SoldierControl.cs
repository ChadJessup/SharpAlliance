...
public class SoldierControl
{
    ...
    public static void EVENT_StopMerc(SOLDIERTYPE? pSoldier, int sGridNo, WorldDirections bDirection)
    {
        ...
        pSoldier.fTurningFromPronePosition = TURNING_FROM_PRONE.OFF;
        ...
        if (pSoldier.fTurningToShoot)
        {
            ...
            Overhead.ReduceAttackBusyCount(pSoldier.ubID, false);
        }
    ...
    public static bool ReevaluateEnemyStance(SOLDIERTYPE pSoldier, AnimationStates usAnimState)
    {
        ...
        {
            if (pSoldier.fTurningFromPronePosition == TURNING_FROM_PRONE.OFF)
            {
    ...
    public static void CheckForFreeupFromHit(SOLDIERTYPE pSoldier, ANIM uiOldAnimFlags, ANIM uiNewAnimFlags, AnimationStates usOldAniState, AnimationStates usNewState)
    {
        ...
        if (usOldAniState == AnimationStates.QUEEN_HIT && usNewState == AnimationStates.QUEEN_HIT)
        {
            return;
        }

        if (usOldAniState != usNewState
            && (uiOldAnimFlags.HasFlag(ANIM.HITSTART))
            && !(uiNewAnimFlags.HasFlag(ANIM.HITFINISH))
            && !(uiNewAnimFlags.HasFlag(ANIM.IGNOREHITFINISH))
            && !(pSoldier.uiStatusFlags.HasFlag(SOLDIER.TURNINGFROMHIT)))
        {
            // Release attacker
        ...
        if ((uiOldAnimFlags.HasFlag(ANIM.HITWHENDOWN))
            && ((pSoldier.uiStatusFlags.HasFlag(SOLDIER.DEAD))
            || pSoldier.IsAlive))
        {
            // Release attacker
    ...
    public static void AdjustNoAPToFinishMove(SOLDIERTYPE pSoldier, bool fSet)
    {
        ...
        if (!fSet)
        {
            ...
            pSoldier.ubReasonCantFinishMove = REASON_STOPPED.NO_APS;
        }
    }
    ...
    public static bool EVENT_InitNewSoldierAnim(SOLDIERTYPE pSoldier, AnimationStates usNewState, int usStartingAniCode, bool fForce)
    {
        ...
        if (!(gTacticalStatus.uiFlags.HasFlag(TacticalEngineStatus.LOADING_SAVED_GAME)))
        {
            ...
            //if ( pSoldier.bLife > INJURED_CHANGE_THREASHOLD )
            {
                    ...
                    && pSoldier.bTeam == gbPlayerNum)
                {
                        ...
                        && !(gAnimControl[pSoldier.usAnimState].uiFlags.HasFlag(ANIM.RAISE_WEAPON | ANIM.NOCHANGE_WEAPON)))
                    {
                        ...
                        if (usItem != NOTHING
                            && (Item[usItem].fFlags.HasFlag(ItemAttributes.ITEM_TWO_HANDED))
            ...
            if (gAnimControl[usNewState].uiFlags.HasFlag(ANIM.MOVING)
                && pSoldier.usAnimState == AnimationStates.COWERING
                && gAnimControl[usNewState].ubEndHeight == AnimationHeights.ANIM_STAND)
            {
                pSoldier.usPendingAnimation = usNewState;
                // Set new state to be animation to move to new stance
            ...
            {
                HandleUI.UnSetUIBusy(pSoldier.ubID);
            }
            ...
            if (pSoldier.uiAnimSubFlags.HasFlag(SUB_ANIM.BIGGUYTHREATENSTANCE))
            {
                if (usNewState == AnimationStates.KNEEL_DOWN && pSoldier.usAnimState != AnimationStates.BIGMERC_CROUCH_TRANS_INTO)
                {
                    ...
                    if (usItem != NOTHING)
                    {
                        if (Item[usItem].usItemClass == IC.GUN && usItem != Items.ROCKET_LAUNCHER)
                        {
            ...
            if (pSoldier.bReverse)
            {
                ...
                {
                    // CHECK FOR SIDEWAYS!
                    if (pSoldier.bDirection == gPurpendicularDirection[pSoldier.bDirection][(int)pSoldier.usPathingData[pSoldier.usPathIndex]])
                    {
            ...
            if (gAnimControl[usNewState].uiFlags.HasFlag(ANIM.MOVING)
                && !(gAnimControl[pSoldier.usAnimState].uiFlags.HasFlag(ANIM.MOVING))
                || (gAnimControl[usNewState].uiFlags.HasFlag(ANIM.MOVING)
                && fForce))
            {
                bool fKeepMoving = false;

                if (usNewState == AnimationStates.CRAWLING && pSoldier.usDontUpdateNewGridNoOnMoveAnimChange == LOCKED_NO_NEWGRIDNO)
                {
                ...
                if (pSoldier.usDontUpdateNewGridNoOnMoveAnimChange == 0)
                {
                    if (usNewState != AnimationStates.SWATTING)
                    {
                        ...
                        if (!(gAnimControl[usNewState].uiFlags.HasFlag(ANIM.SPECIALMOVE)))
                        {
                            // Handle goto new tile...
                            //                            if (HandleGotoNewGridNo(pSoldier, out fKeepMoving, true, usNewState))
                            {
                                if (!fKeepMoving)
                                {
                                    return (false);
                            ...
                            //                            else
                            {
                                if (pSoldier.bBreathCollapsed > 0)
                                {
                                    // UNset UI
                                    HandleUI.UnSetUIBusy(pSoldier.ubID);
        ...
        if (uiNewAnimFlags.HasFlag(ANIM.NONINTERRUPT))
        {
            pSoldier.fInNonintAnim = true;
        }

        if (uiNewAnimFlags.HasFlag(ANIM.RT_NONINTERRUPT))
        {
        ...
        if (!(gAnimControl[pSoldier.usAnimState].uiFlags.HasFlag(ANIM.FIREREADY))
            && !(gAnimControl[usNewState].uiFlags.HasFlag(ANIM.FIREREADY)))
        {
            // ATE: Also check for the transition anims to not reset this
            // this should have used a flag but we're out of them....
            if (usNewState != AnimationStates.READY_RIFLE_STAND
               && usNewState != AnimationStates.READY_RIFLE_PRONE
               && usNewState != AnimationStates.READY_RIFLE_CROUCH
               && usNewState != AnimationStates.ROBOT_SHOOT)
            {
        ...
        if (gAnimControl[usNewState].uiFlags.HasFlag(ANIM.UPDATEMOVEMENTMODE))
        {
        ...
        if (pSoldier.ubID < MAX_NUM_SOLDIERS)
        {
            ...
            switch (usNewState)
            {
                ...
                case AnimationStates.READY_DUAL_PRONE:
                    ...
                    {
                        sAPCost = Points.GetAPsToReadyWeapon(pSoldier, usNewState);
                ...
                case AnimationStates.PRONE_DOWN:
                    ...
                    if (pSoldier.fTurningFromPronePosition < TURNING_FROM_PRONE.START_UP_FROM_MOVE && !pSoldier.fDontChargeAPsForStanceChange)
                    {
                ...
                case AnimationStates.CROW_DIE:
                    ...
                    {
                        TileAnimations.DeleteAniTile(pSoldier.pAniTile);
                ...
                case AnimationStates.JUMP_OVER_BLOCKING_PERSON:
                    ...
                    // Set path....
                    {
                        ...
                        // Set direction
                        EVENT_InternalSetSoldierDestination(pSoldier, pSoldier.usPathingData[pSoldier.usPathIndex], false, AnimationStates.JUMP_OVER_BLOCKING_PERSON);
                    }
                    break;
        ...
        if (AnimationControl.SetSoldierAnimationSurface(pSoldier, usNewState) == false)
        {
            return (false);
        }
        ...
        if (usNewState == AnimationStates.CHARIOTS_OF_FIRE || usNewState == AnimationStates.BODYEXPLODING)
        {
            pSoldier.usAniCode = (Globals.Random.Next(10));
        }
        ...
        {
            // Are we stationary?
            if (gAnimControl[usNewState].uiFlags.HasFlag(ANIM.STATIONARY))
            {
                // Position light....
                // SetCheckSoldierLightFlag( pSoldier );
            }
        ...
        if (usNewState == AnimationStates.VEHICLE_DIE || usNewState == AnimationStates.CHARIOTS_OF_FIRE || usNewState == AnimationStates.BODYEXPLODING)
        {
        ...
        if (!(gTacticalStatus.uiFlags.HasFlag(TacticalEngineStatus.LOADING_SAVED_GAME)))
        {
            if ((gAnimControl[pSoldier.usAnimState].uiFlags.HasFlag(ANIM.STATIONARY))
                && (gAnimControl[pSoldier.usOldAniState].uiFlags.HasFlag(ANIM.MOVING)))
            {
        ...
        AIMain.FreeUpNPCFromStanceChange(pSoldier);

        return (true);
    }


    public static void InternalRemoveSoldierFromGridNo(SOLDIERTYPE? pSoldier, bool fForce)
    {
        int bDir;
        int iGridNo;
        ...
        if ((pSoldier.sGridNo != NO_MAP_POS))
        {
            if (pSoldier.bInSector || fForce)
            {
                ...
                {
                    iGridNo = pSoldier.sGridNo + IsometricUtils.DirIncrementer[bDir];
                ...
                SoldierTile.UnMarkMovementReserved(pSoldier);
    ...
    public static void RemoveSoldierFromGridNo(SOLDIERTYPE pSoldier)
    {
    ...
    public static void InternalSetSoldierHeight(SOLDIERTYPE pSoldier, float dNewHeight, bool fUpdateLevel)
    {
    ...
    public static void SetSoldierHeight(SOLDIERTYPE pSoldier, float dNewHeight)
    {
    ...
    public static void SetSoldierGridNo(SOLDIERTYPE pSoldier, int sNewGridNo, bool fForceRemove)
    {
        ...
        if (sNewGridNo != pSoldier.sGridNo || pSoldier.pLevelNode == null)
        {
            ...
            if (gAnimControl[pSoldier.usAnimState].uiFlags.HasFlag(ANIM.MOVING | ANIM.SPECIALMOVE))
            {
                if (!(gTacticalStatus.uiFlags.HasFlag(TacticalEngineStatus.LOADING_SAVED_GAME)))
                {
            ...
            {
                if (guiCurrentEvent == UI_EVENT_DEFINES.C_WAIT_FOR_CONFIRM)
                {
                    // Update path!
            ...
            if (pSoldier.bEndDoorOpenCode == 2)
            {
                ...
                HandleDoors.HandleDoorChangeFromGridNo(pSoldier, pSoldier.sEndDoorOpenCodeData, false);
            }
            ...
            for (bDir = 0; bDir < NUM_WORLD_DIRECTIONS; bDir++)
            {
                gpWorldLevelData[pSoldier.sGridNo + IsometricUtils.DirIncrementer[bDir]].ubAdjacentSoldierCnt++;
            }

            if (!(pSoldier.uiStatusFlags.HasFlag(SOLDIER.DRIVER | SOLDIER.PASSENGER)))
            {
                Smell.DropSmell(pSoldier);
            }
            ...
            if (StructureWrap.IsJumpableFencePresentAtGridno(sNewGridNo))
            {
                //sX = MapX( sNewGridNo );
                //sY = MapY( sNewGridNo );
            ...
            if (!(pSoldier.uiStatusFlags.HasFlag(SOLDIER.DRIVER | SOLDIER.PASSENGER)))
            {
                ...
                if (pSoldier.usAnimState == AnimationStates.CLIMBUPROOF)
                {
                ...
                else if (pSoldier.usAnimState == AnimationStates.CLIMBDOWNROOF)
                {
                    if (pSoldier.iLight != (-1))
                    {
            ...
            if (!(pSoldier.uiStatusFlags.HasFlag(SOLDIER.DRIVER | SOLDIER.PASSENGER)))
            {
                ...
                if (fInWaterValue != pSoldier.fPrevInWater)
                {
                    ...
                    AnimationControl.SetSoldierAnimationSurface(pSoldier, pSoldier.usAnimState);
                ...
                if (pSoldier.bOverTerrainType == TerrainTypeDefines.DEEP_WATER
                    && pSoldier.bOldOverTerrainType != TerrainTypeDefines.DEEP_WATER)
                {
                    ...
                    switch (pSoldier.usAnimState)
                    {
                        ...
                        case AnimationStates.RUNNING:
                            ...
                            EVENT_InitNewSoldierAnim(pSoldier, AnimationStates.LOW_TO_DEEP_WATER, 0, false);
                            pSoldier.usPendingAnimation = AnimationStates.DEEP_WATER_SWIM;
                ...
                if (pSoldier.bOverTerrainType == TerrainTypeDefines.MED_WATER || pSoldier.bOverTerrainType == TerrainTypeDefines.DEEP_WATER)
                {
                    //                    WaterDamage(pSoldier);
                }

                // OK, If we were in deep water but we are NOT now, handle mid animations!
                if (pSoldier.bOverTerrainType != TerrainTypeDefines.DEEP_WATER && pSoldier.bOldOverTerrainType == TerrainTypeDefines.DEEP_WATER)
                {
                    // Make transition from low to deep
                    EVENT_InitNewSoldierAnim(pSoldier, AnimationStates.DEEP_TO_LOW_WATER, 0, false);
            ...
            //            if (GetSmokeEffectOnTile(sNewGridNo, pSoldier.bLevel))
            {
                ...
                if (pSoldier.inv[InventorySlot.HEAD1POS].usItem == Items.GASMASK && pSoldier.inv[InventorySlot.HEAD1POS].bStatus[0] >= GASMASK_MIN_STATUS)
                {
                    fSetGassed = false;
                }
                if (pSoldier.inv[InventorySlot.HEAD2POS].usItem == Items.GASMASK && pSoldier.inv[InventorySlot.HEAD2POS].bStatus[0] >= GASMASK_MIN_STATUS)
                {
                    fSetGassed = false;
                }
    ...
    public static void EVENT_FireSoldierWeapon(SOLDIERTYPE? pSoldier, int sTargetGridNo)
    {
        ...
        if (Item[pSoldier.inv[InventorySlot.HANDPOS].usItem].usItemClass.HasFlag(IC.GUN))
        {
            if (pSoldier.bDoBurst)
            {
                ...
                pSoldier.bBulletsLeft = Math.Min(WeaponTypes.Weapon[pSoldier.inv[pSoldier.ubAttackingHand].usItem].ubShotsPerBurst, pSoldier.inv[pSoldier.ubAttackingHand].ubGunShotsLeft);
            }
            ...
            if (pSoldier.inv[pSoldier.ubAttackingHand].ubGunAmmoType == AMMO.BUCKSHOT)
            {
...
//                    if (sDist <= 2 && dTargetZ <= 100)
    ...
    public static void SelectFallAnimation(SOLDIERTYPE pSoldier)
    {
        // Determine which animation to do...depending on stance and gun in hand...
        switch (gAnimControl[pSoldier.usAnimState].ubEndHeight)
        {
        ...
        else if (Item[usWeaponIndex].usItemClass.HasFlag((IC.GUN | IC.THROWING_KNIFE)))
        {
            ...
            else if (ubSpecial == FIRE_WEAPON_BLINDED_BY_SPIT_SPECIAL)
            {
                ...
                if ((pSoldier.bBlindedCounter == 0))
                {
                    ...
                    OppList.DecayIndividualOpplist(pSoldier);
                }
...
//                pNewSoldier = ReduceAttackBusyGivenTarget(pSoldier.ubAttackerID, pSoldier.ubID);
        ...
        if (pSoldier.uiStatusFlags.HasFlag(SOLDIER.VEHICLE))
        {
            SoldierTakeDamage(pSoldier, AnimationHeights.ANIM_CROUCH, sDamage, sBreathLoss, ubReason, pSoldier.ubAttackerID, NOWHERE, 0, true);
            return;
        }

        // DEDUCT LIFE
        ubCombinedLoss = SoldierTakeDamage(pSoldier, AnimationHeights.ANIM_CROUCH, sDamage, sBreathLoss, ubReason, pSoldier.ubAttackerID, NOWHERE, 0, true);
        ...
        if (pSoldier.bTeam == gbPlayerNum)
        {
            if (pSoldier.bAssignment >= Assignments.ON_DUTY && pSoldier.bAssignment != Assignments.ASSIGNMENT_POW)
            {
                ...
                Squads.AddCharacterToAnySquad(pSoldier);
            }
        }
        ...
        if (pSoldier.bLife >= CONSCIOUSNESS)
        {
            ...
            else
            {
                ...
                if (pSoldier.fGettingHit > 0 && pSoldier.usAniCode != (int)AnimationStates.STANDING_BURST_HIT)
                {

                }
                else
                {
                    //                    DoMercBattleSound(pSoldier, (BATTLE_SOUND.HIT1 + Globals.Random.Next(2)));
                }
            }
        }
        ...
        switch (pSoldier.ubBodyType)
        {
            ...
            case SoldierBodyTypes.YAM_MONSTER:
                ...
                EVENT_InitNewSoldierAnim(pSoldier, AnimationStates.ADULTMONSTER_HIT, 0, false);
                return;
                break;

            case SoldierBodyTypes.LARVAE_MONSTER:
                EVENT_InitNewSoldierAnim(pSoldier, AnimationStates.LARVAE_HIT, 0, false);
                return;
                break;

            case SoldierBodyTypes.QUEENMONSTER:
                EVENT_InitNewSoldierAnim(pSoldier, AnimationStates.QUEEN_HIT, 0, false);
                return;
                break;
            ...
            case SoldierBodyTypes.CRIPPLECIV:

                {
                    ...
                    EVENT_InitNewSoldierAnim(pSoldier, AnimationStates.CRIPPLE_HIT, 0, false);
            ...
            case SoldierBodyTypes.ROBOTNOWEAPON:
                EVENT_InitNewSoldierAnim(pSoldier, AnimationStates.ROBOTNW_HIT, 0, false);
                return;
                break;


            case SoldierBodyTypes.INFANT_MONSTER:
                EVENT_InitNewSoldierAnim(pSoldier, AnimationStates.INFANT_HIT, 0, false);
                return;

            case SoldierBodyTypes.CROW:

                EVENT_InitNewSoldierAnim(pSoldier, AnimationStates.CROW_DIE, 0, false);
                return;
            ...
            case SoldierBodyTypes.KIDCIV:
                ...
                if (!(pSoldier.uiStatusFlags.HasFlag(SOLDIER.DEAD)) && pSoldier.bLife == 0)
                {
                    ...
                    {
                        EVENT_InitNewSoldierAnim(pSoldier, AnimationStates.CIV_DIE2, 0, false);
                        return;
                    }
                }
        ...
        {
            EVENT_InitNewSoldierAnim(pSoldier, AnimationStates.WATER_HIT, 0, false);
            return;
        }
        if (pSoldier.bOverTerrainType == TerrainTypeDefines.DEEP_WATER)
        {
            EVENT_InitNewSoldierAnim(pSoldier, AnimationStates.DEEP_WATER_HIT, 0, false);
            return;
        }


        // SWITCH IN TYPE OF WEAPON
        if (Item[usWeaponIndex].usItemClass.HasFlag(IC.GUN | IC.THROWING_KNIFE))
        {
        ...
        if (Item[usWeaponIndex].usItemClass.HasFlag(IC.EXPLOSV) || Item[usWeaponIndex].usItemClass.HasFlag(IC.TENTACLES))
        {
    ...
    public static int CalcScreamVolume(SOLDIERTYPE pSoldier, int ubCombinedLoss)
    {
    ...
    public static void DoGenericHit(SOLDIERTYPE pSoldier, int ubSpecial, WorldDirections bDirection)
    {
        ...
        switch (gAnimControl[pSoldier.usAnimState].ubEndHeight)
        {
            case AnimationHeights.ANIM_STAND:
                ...
                if (ubSpecial == FIRE_WEAPON_BURST_SPECIAL && pSoldier.ubBodyType <= SoldierBodyTypes.REGFEMALE)
                {
    ...
    public static void SoldierGotHitGunFire(SOLDIERTYPE? pSoldier, Items usWeaponIndex, int sDamage, WorldDirections bDirection, int sRange, int ubAttackerID, int ubSpecial, int ubHitLocation)
    {
        int usNewGridNo;
                ...
                if (ubSpecial == FIRE_WEAPON_HEAD_EXPLODE_SPECIAL)
                {
                    if (GameSettings.fOptions[TOPTION.BLOOD_N_GORE])
                    {
                        if (IsometricUtils.SpacesAway(pSoldier.sGridNo, Menptr[ubAttackerID].sGridNo) <= MAX_DISTANCE_FOR_MESSY_DEATH)
                        {
                            ...
                            if (SoldierAni.OKFallDirection(pSoldier, usNewGridNo, pSoldier.bLevel, pSoldier.bDirection, AnimationStates.JFK_HITDEATH))
                            {
                                usNewGridNo = IsometricUtils.NewGridNo((int)usNewGridNo, (int)(IsometricUtils.DirectionInc(pSoldier.bDirection)));

                                if (SoldierAni.OKFallDirection(pSoldier, usNewGridNo, pSoldier.bLevel, pSoldier.bDirection, pSoldier.usAnimState))
                                {
                                    fHeadHit = true;
                                }
                            }
                    ...
                    if (GameSettings.fOptions[TOPTION.BLOOD_N_GORE])
                    {
                        if (IsometricUtils.SpacesAway(pSoldier.sGridNo, Menptr[ubAttackerID].sGridNo) <= MAX_DISTANCE_FOR_MESSY_DEATH)
                        {
                            ...
                            if (pSoldier.bDirection == bDirection)
                            {
                                ...
                                if (SoldierAni.OKFallDirection(pSoldier, usNewGridNo, pSoldier.bLevel, gOppositeDirection[bDirection], AnimationStates.FLYBACK_HIT))
                                {
                                    usNewGridNo = IsometricUtils.NewGridNo(usNewGridNo, IsometricUtils.DirectionInc(gOppositeDirection[bDirection]));

                                    if (SoldierAni.OKFallDirection(pSoldier, usNewGridNo, pSoldier.bLevel, gOppositeDirection[bDirection], pSoldier.usAnimState))
                                    {
                                        fBlownAway = true;
                                    }
                                }
    ...
    public static void SoldierGotHitExplosion(SOLDIERTYPE pSoldier, Items usWeaponIndex, int sDamage, WorldDirections bDirection, int sRange, int ubAttackerID, int ubSpecial, int ubHitLocation)
    {
        int sNewGridNo;
        ...
        if (GameSettings.fOptions[TOPTION.BLOOD_N_GORE])
        {
            ...
            //                && pSoldier.bLife == 0
            //                && gAnimControl[pSoldier.usAnimState].ubEndHeight != AnimationHeights.ANIM_PRONE)
            {
    ...
    public static void SoldierGotHitBlade(SOLDIERTYPE? pSoldier, Items usWeaponIndex, int sDamage, WorldDirections bDirection, int sRange, int ubAttackerID, int ubSpecial, int ubHitLocation)
    {
    ...
    public static void SoldierGotHitPunch(SOLDIERTYPE pSoldier, Items usWeaponIndex, int sDamage, WorldDirections bDirection, int sRange, int ubAttackerID, int ubSpecial, int ubHitLocation)
    {
    ...
    public static bool EVENT_InternalGetNewSoldierPath(SOLDIERTYPE pSoldier, int sDestGridNo, AnimationStates usMovementAnim, int fFromUI, bool fForceRestartAnim)
    {
        ...
        int sMercGridNo;
        WorldDirections[] usPathingData = new WorldDirections[MAX_PATH_LIST_SIZE];
        WorldDirections ubPathingMaxDirection;
            ...
            && (gAnimControl[pSoldier.usAnimState].uiFlags.HasFlag(ANIM.MOVING)) && fFromUI == 1) || fFromUI == 2)
        {
            ...
            if (uiDist > 0)
            {
                ...
                if (fAdvancePath)
                {
                    usPathingData = pSoldier.usPathingData;
                    ubPathingMaxDirection = usPathingData[MAX_PATH_LIST_SIZE - 1];
                    //                    pSoldier.usPathingData[1..] = usPathingData[..^1];
...
//            fContinue = (iDest != 0);
    ...
    public static void SoldierGotoStationaryStance(SOLDIERTYPE? pSoldier)
    {
        ...
        {
            // IN deep water, tred!
            EVENT_InitNewSoldierAnim(pSoldier, AnimationStates.DEEP_WATER_TRED, 0, false);
        }
        else if (pSoldier.ubServicePartner != NOBODY && pSoldier.bLife >= OKLIFE && pSoldier.bBreath > 0)
        {
            EVENT_InitNewSoldierAnim(pSoldier, AnimationStates.GIVING_AID, 0, false);
        }
        else
        {
            ...
            switch (gAnimControl[pSoldier.usAnimState].ubEndHeight)
            {
                ...
                case AnimationHeights.ANIM_CROUCH:
                    ...
                    if (pSoldier.uiStatusFlags.HasFlag(SOLDIER.COWERING))
                    {
                        EVENT_InitNewSoldierAnim(pSoldier, AnimationStates.COWERING, 0, false);
                    }
                    else
                    {
                        EVENT_InitNewSoldierAnim(pSoldier, AnimationStates.CROUCHING, 0, false);
                    }
                    break;

                case AnimationHeights.ANIM_PRONE:
                    EVENT_InitNewSoldierAnim(pSoldier, AnimationStates.PRONE, 0, false);
                    break;
            }
    ...
    public static void EVENT_InternalSetSoldierDestination(SOLDIERTYPE pSoldier, WorldDirections usNewDirection, bool fFromMove, AnimationStates usAnimState)
    {
        int usNewGridNo;
        int sXPos, sYPos;

        // Get dest gridno, convert to center coords
        usNewGridNo = IsometricUtils.NewGridNo(pSoldier.sGridNo, IsometricUtils.DirectionInc(usNewDirection));
...
//        pSoldier.sDestYPos = sYPos;
        ...
        {
            WorldDirections ubPerpDirection;

            // Get a new desired direction, 
            ubPerpDirection = gPurpendicularDirection[pSoldier.bDirection][(int)usNewDirection];
        ...
        {
            if (!(gAnimControl[usAnimState].uiFlags.HasFlag(ANIM.SPECIALMOVE)))
            {
    ...
    public static void EVENT_SetSoldierDestination(SOLDIERTYPE pSoldier, WorldDirections usNewDirection)
    {
        SoldierControl.EVENT_InternalSetSoldierDestination(pSoldier, usNewDirection, false, pSoldier.usAnimState);
    }


    // function to determine which direction a creature can turn in
    public static int MultiTiledTurnDirection(SOLDIERTYPE pSoldier, WorldDirections bStartDirection, WorldDirections bDesiredDirection)
    {
        int bTurningIncrement = 0;
    ...
    public static void EVENT_InternalSetSoldierDesiredDirection(SOLDIERTYPE? pSoldier, WorldDirections usNewDirection, bool fInitalMove, AnimationStates usAnimState)
    {
        ...
        if (pSoldier.bDesiredDirection != pSoldier.bDirection)
        {
            ...
            if (fInitalMove)
            {
                ...
                {
                    if (pSoldier.fTurningFromPronePosition != TURNING_FROM_PRONE.ENDING_UP_FROM_MOVE)
                    {
                        pSoldier.fTurningFromPronePosition = TURNING_FROM_PRONE.START_UP_FROM_MOVE;
                    }
                }
            }
                ...
                || pSoldier.fNoAPToFinishMove || fInitalMove)
            {
                if (gAnimControl[usAnimState].ubHeight == AnimationHeights.ANIM_PRONE)
                {
                    ...
                    if (!fInitalMove)
                    {
                        pSoldier.fTurningFromPronePosition = TURNING_FROM_PRONE.ON;
    ...
    public static void EVENT_SetSoldierDesiredDirection(SOLDIERTYPE? pSoldier, WorldDirections usNewDirection)
    {
    ...
    public static void EVENT_BeginMercTurn(SOLDIERTYPE? pSoldier, bool fFromRealTime, int iRealTimeCounter)
    {
        ...
        if (!gTacticalStatus.fAutoBandageMode)
        {
            ...
            {
                //                if (pSoldier.IsBleeding || pSoldier.bLife < OKLIFE) // is he bleeding or dying?
                {
        ...
        if (pSoldier.bLife > 0 && pSoldier.bBreath == 0 && MercInWater(pSoldier))
        {
            // Drowning...
        }
        ...
        if (pSoldier.bLife > 0)
        {
            ...
            {
                if (pSoldier.ubNoiseVolume > 0)    // and the noise volume is still positive
                {
                    pSoldier.ubNoiseVolume--;  // the volume of the noise "decays" by 1 point

                    if (pSoldier.ubNoiseVolume == 0)   // if the volume has reached zero
                    {
        ...
        if ((savedPts = pSoldier.bActionPts) > MAX_AP.CARRIED)
          savedPts = MAX_AP.CARRIED;
            ...
            if (pSoldier.uiStatusFlags.HasFlag(SOLDIER.GASSED))
            {
                ...
                if ((pSoldier.inv[InventorySlot.HEAD1POS].usItem == Items.GASMASK
            ...
            if (pSoldier.bBlindedCounter > 0)
            {
                ...
                {
                    // we can SEE!!!!!
                    OppList.HandleSight(pSoldier, SIGHT.LOOK);
                    // Dirty panel
                    fInterfacePanelDirty = DIRTYLEVEL2;
                }
            }
            ...
            pSoldier.sWeightCarriedAtTurnStart = ItemSubSystem.CalculateCarriedWeight(pSoldier);
            ...
            if (pSoldier.bInSector)
            {
                ...
                if (gTacticalStatus.uiFlags.HasFlag(TacticalEngineStatus.INCOMBAT))
                {
                    OppList.VerifyAndDecayOpplist(pSoldier);

                    // turn off xray
                    if (pSoldier.uiXRayActivatedTime > 0)
                    {
                ...
                if ((pSoldier.bTeam == gbPlayerNum) && (pSoldier.ubProfile != NO_PROFILE))
                {
                    switch (gMercProfiles[pSoldier.ubProfile].bPersonalityTrait)
                    {
                        case PersonalityTrait.FEAR_OF_INSECTS:
                            if (OppList.MercSeesCreature(pSoldier))
                            {
    ...
    public static bool ConvertAniCodeToAniFrame(SOLDIERTYPE pSoldier, int usAniFrame)
    {
        ...
        //if (gAnimSurfaceDatabase[usAnimSurface].ubFlags & ANIM_DATA_FLAG_OFRAMES)
        //{
        //    usAniFrame = 0;
        //}
        //
    ...
    public static void TurnSoldier(SOLDIERTYPE pSoldier)
    {
        WorldDirections sDirection;
        ...
        if (pSoldier.uiStatusFlags.HasFlag(SOLDIER.LOOK_NEXT_TURNSOLDIER))
        {
            ...
            {
                // HANDLE SIGHT!
                OppList.HandleSight(pSoldier, SIGHT.LOOK | SIGHT.RADIO);
            }
            // Turn off!
        ...
        if (pSoldier.fTurningToShoot)
        {
            ...
            {
                if (((gAnimControl[pSoldier.usAnimState].uiFlags.HasFlag(ANIM.FIREREADY)) && pSoldier.fTurningFromPronePosition == TURNING_FROM_PRONE.OFF)
                    || pSoldier.ubBodyType == SoldierBodyTypes.ROBOTNOWEAPON
                    || pSoldier.ubBodyType == SoldierBodyTypes.TANK_NW
                    || pSoldier.ubBodyType == SoldierBodyTypes.TANK_NE)
                {
                    EVENT_InitNewSoldierAnim(pSoldier, SelectFireAnimation(pSoldier, gAnimControl[pSoldier.usAnimState].ubEndHeight), 0, false);
                    pSoldier.fTurningToShoot = false;
                ...
                else if (pSoldier.fTurningFromPronePosition != TURNING_FROM_PRONE.OFF)
                {
                    if (IsValidStance(pSoldier, AnimationHeights.ANIM_PRONE))
                    {
        ...
        if (pSoldier.fTurningUntilDone && (pSoldier.usPendingAnimation != NO_PENDING_ANIMATION))
        {
            if (pSoldier.bDirection == pSoldier.bDesiredDirection)
            {
                AnimationStates usPendingAnimation;
        ...
        if (pSoldier.bDirection == pSoldier.bDesiredDirection)
        {
            ...
            if (pSoldier.uiStatusFlags.HasFlag(SOLDIER.VEHICLE) || CREATURE_OR_BLOODCAT(pSoldier))
            {
                pSoldier.uiStatusFlags &= (~SOLDIER.PAUSEANIMOVE);
            }
            ...
            // if (pSoldier.fTurningFromPronePosition == TURNING_FROM_PRONE_ON)
            {
                ...
                if (IsValidStance(pSoldier, AnimationHeights.ANIM_PRONE) && pSoldier.pTempObject == null)
                {
                    SendChangeSoldierStanceEvent(pSoldier, AnimationHeights.ANIM_PRONE);
                }
                // pSoldier.fTurningFromPronePosition = TURNING_FROM_PRONE_OFF;
            ...
            //if (pSoldier.fTurningFromPronePosition == TURNING_FROM_PRONE_ENDING_UP_FROM_MOVE
            //    && pSoldier.usAnimState != AnimationStates.PRONE_UP
            //    && pSoldier.usAnimState != AnimationStates.PRONE_DOWN)
            {
                if (IsValidStance(pSoldier, AnimationHeights.ANIM_PRONE))
                {
                    EVENT_InitNewSoldierAnim(pSoldier, AnimationStates.CRAWLING, 0, false);
                }
            }
            ...
            if (pSoldier.uiStatusFlags.HasFlag(SOLDIER.TURNINGFROMHIT))
            {
                ...
                {
                    if (pSoldier.usPendingAnimation != AnimationStates.FALLFORWARD_ROOF
                        && pSoldier.usPendingAnimation != AnimationStates.FALLOFF
                        && pSoldier.usAnimState != AnimationStates.FALLFORWARD_ROOF
                        && pSoldier.usAnimState != AnimationStates.FALLOFF)
                    {
                        // Go back to original direction
                        EVENT_SetSoldierDesiredDirection(pSoldier, (WorldDirections)pSoldier.uiPendingActionData1);
        ...
        else
        {
            ...
            if (sDirection > (WorldDirections)7)
            {
                sDirection = 0;
            }
            else
        ...
        if (fDoDirectionChange)
        {
            // if (OKToAddMercToWorld(pSoldier, (int)sDirection))
            {
                ...
                if (pSoldier.ubBodyType != SoldierBodyTypes.LARVAE_MONSTER
                    && !MercInWater(pSoldier)
                    && pSoldier.bOverTerrainType != TerrainTypeDefines.DIRT_ROAD
                    && pSoldier.bOverTerrainType != TerrainTypeDefines.PAVED_ROAD)
                {
                    PlaySoldierFootstepSound(pSoldier);
                }
            }
...
int	gOrangeGlowG[]=
{
    ...
    public static int[] gOrangeGlowG =
    {
    0,			// Normal shades
	20,
    40,
    ...
    {
        AnimationSurfaceTypes usAnimSurface, usPaletteAnimSurface = 0;
        string zColFilename;// [100];
        int iWhich;
        int cnt;
        int bBodyTypePalette = 0;
    ...
    public static void CalculateSoldierAniSpeed(SOLDIERTYPE pSoldier, SOLDIERTYPE pStatsSoldier)
    {
        ...
        int bBreathDef = 0, bLifeDef, bAgilDef;
        int bAdditional = 0;
...
//        uiTerrainDelay += (bLifeDef + bBreathDef + bAgilDef + bAdditional);
    ...
    public static void SetSoldierAniSpeed(SOLDIERTYPE pSoldier)
    {
        ...
        {
            if (((pSoldier.bVisible == -1 && pSoldier.bVisible == pSoldier.bLastRenderVisibleValue) || gTacticalStatus.fAutoBandageMode)
                && pSoldier.usAnimState != AnimationStates.MONSTER_UP)
            {
...
//        if (_KeyDown(SPACE))
    ...
    public static bool SetPaletteReplacement(List<SGPPaletteEntry> p8BPPPalette, PaletteRepID aPalRep)
    {
        int cnt2;
        int ubType;
...
//        }
    ...
    public static bool GetPaletteRepIndexFromID(PaletteRepID aPalRep, out int pubPalIndex)
    {
        int cnt;
...
//        }
    ...
    void MoveMercFacingDirection(SOLDIERTYPE pSoldier, bool fReverse, float dMovementDist)
    {
        double dAngle = 0;
    ...
    {
        WorldDirections bNewDirection = 0;
...
//        if (FindFenceJumpDirection(pSoldier, pSoldier.sGridNo, pSoldier.bDirection, out bDirection))
    ...
    public static int SleepDartSuccumbChance(SOLDIERTYPE pSoldier)
    {
        int uiChance;
    ...
    public static void BeginSoldierGetup(SOLDIERTYPE pSoldier)
    {
        ...
        if (pSoldier.bCollapsed)
        {
            ...
            else
            {
                ...
                if ((gTacticalStatus.bBoxingState == BoxingStates.BOXING) && (pSoldier.uiStatusFlags.HasFlag(SOLDIER.BOXER)))
                {
            ...
            case TAKE_DAMAGE.GAS:
                ...
                if (pSoldier.bInSector)
                {
                    if (pSoldier.bVisible != -1)
                    {
                        ...
                        {
                            DoMercBattleSound(pSoldier, BATTLE_SOUND.DIE1);
    ...
    public static int SoldierTakeDamage(SOLDIERTYPE pSoldier, AnimationHeights bHeight, int sLifeDeduct, int sBreathLoss, TAKE_DAMAGE ubReason, int ubAttacker, int sSourceGrid, int sSubsequent, bool fShowDamage)
    {
        ...
        switch (pSoldier.bTeam)
        {
            case TEAM.ENEMY_TEAM:
                ...
                if (strategicMap[CALCULATE_STRATEGIC_INDEX(gWorldSectorX, gWorldSectorY)].fEnemyControlled || SectorInfo[SECTORINFO.SECTOR(gWorldSectorX, gWorldSectorY)].ubTraversability[StrategicMove.THROUGH] != Traversability.TOWN)
                {
...
//                pSoldier.bBleeding = pSoldier.bLifeMax - (pSoldier.bLife + bBandage);
        ...
        {
            Points.DeductPoints(pSoldier, sAPCost, sBreathLoss);
        }
...
//                            DelayedTacticalCharacterDialogue(pSoldier, QUOTE.TAKEN_A_BREATING);
    ...
    public static bool InternalDoMercBattleSound(SOLDIERTYPE? pSoldier, BATTLE_SOUND ubBattleSoundID, int bSpecialCode)
    {
        string zFilename;
        //        SOUNDPARMS spParms;
        BATTLE_SOUND ubSoundID;
        int uiSoundID;
        int iFaceIndex;
        bool fDoSub = false;
        ...
        // doubleCHECK RANGE
        CHECKF(ubBattleSoundID < BATTLE_SOUND.NUM_MERC_BATTLE_SOUNDS);
        ...
        if (ubBattleSoundID == BATTLE_SOUND.DIE1)
        {
        ...
        if (ubBattleSoundID == BATTLE_SOUND.HIT1 || ubBattleSoundID == BATTLE_SOUND.HIT2)
        {
            switch (pSoldier.ubBodyType)
            {
                case SoldierBodyTypes.COW:
        ...
        if (ubBattleSoundID == BATTLE_SOUND.OK1 && pSoldier.bMorale < LOW_MORALE_BATTLE_SND_THREASHOLD)
        {
            ubBattleSoundID = BATTLE_SOUND.LOWMARALE_OK1;
        }
        if (ubBattleSoundID == BATTLE_SOUND.ATTN1 && pSoldier.bMorale < LOW_MORALE_BATTLE_SND_THREASHOLD)
        {
            ubBattleSoundID = BATTLE_SOUND.LOWMARALE_ATTN1;
        }

        ubSoundID = ubBattleSoundID;

        //if the sound to be played is a confirmation, check to see if we are to play it
        if (ubSoundID == BATTLE_SOUND.OK1)
        {
...
//            if (!files.FileExists(zFilename))
        ...
        else
        {
            ...
            {
                if (ubSoundID == BATTLE_SOUND.DIE1)
                {
    ...
    public static bool DoMercBattleSound(SOLDIERTYPE? pSoldier, BATTLE_SOUND ubBattleSoundID)
    {
    ...
    bool PreloadSoldierBattleSounds(SOLDIERTYPE? pSoldier, bool fRemove)
    {
        ...
        for (cnt = 0; cnt < BATTLE_SOUND.NUM_MERC_BATTLE_SOUNDS; cnt++)
        {
            // OK, build file and play!
            if (pSoldier.ubProfile != NO_PROFILE)
            {
    ...
    public static int CalcActionPoints(SOLDIERTYPE pSold)
    {
        int ubPoints = 0, ubMaxAPs = 0;
        Items bBandage = 0;
...
//        }
        ...
        if (pSold.ubBodyType == SoldierBodyTypes.BLOODCAT)
        {
            // use same as young monsters
            ubPoints = (ubPoints * AP.YOUNG_MONST_FACTOR) / 10;
        }
        else if (pSold.uiStatusFlags.HasFlag(SOLDIER.MONSTER))
        {
            // young monsters get extra APs
            if (pSold.ubBodyType == SoldierBodyTypes.YAF_MONSTER || pSold.ubBodyType == SoldierBodyTypes.YAM_MONSTER || pSold.ubBodyType == SoldierBodyTypes.INFANT_MONSTER)
            {
                ubPoints = (ubPoints * AP.YOUNG_MONST_FACTOR) / 10;
            }

            // if frenzied, female monsters get more APs! (for young females, cumulative!)
            if (pSold.bFrenzied > 0)
            {

                ubPoints = (ubPoints * AP.MONST_FRENZY_FACTOR) / 10;
            }
        }
        ...
        {
            if ((gMercProfiles[pSold.ubProfile].bPersonalityTrait == PersonalityTrait.CLAUSTROPHOBIC) && (gbWorldSectorZ > 0))
            {
                ubPoints = (ubPoints * AP.CLAUSTROPHOBE) / 10;
            }
            //            else if ((gMercProfiles[pSold.ubProfile].bPersonalityTrait == PersonalityTrait.FEAR_OF_INSECTS) && (MercSeesCreature(pSold)))
            {
                ubPoints = (ubPoints * AP.AFRAID_OF_INSECTS) / 10;
            }
        }

        // Adjusat APs due to drugs...
        //        HandleAPEffectDueToDrugs(pSold, out ubPoints);

        // If we are a vehicle, adjust APS...
        if (pSold.uiStatusFlags.HasFlag(SOLDIER.VEHICLE))
        {
            //            AdjustVehicleAPs(pSold, out ubPoints);
        }


        // if we are in boxing mode, adjust APs... THIS MUST BE LAST!
        if (gTacticalStatus.bBoxingState == BoxingStates.BOXING || gTacticalStatus.bBoxingState == BoxingStates.PRE_BOXING)
        {
            ubPoints /= 2;
        }
    ...
    public static void BeginSoldierClimbDownRoof(SOLDIERTYPE? pSoldier)
    {
...
//            }
    ...
    bool CheckForFullStruct(int sGridNo, out TileIndexes? pusIndex)
    {
    ...
    public static void AdjustForFastTurnAnimation(SOLDIERTYPE pSoldier)
    {
        ...
        if (gAnimControl[pSoldier.usAnimState].uiFlags.HasFlag(ANIM.FASTTURN)
            && pSoldier.bTeam == gbPlayerNum
            && !(pSoldier.uiStatusFlags.HasFlag(SOLDIER.TURNINGFROMHIT)))
        {
            if (pSoldier.bDirection != pSoldier.bDesiredDirection)
    ...
    {
        if (gAnimControl[pSoldier.usAnimState].uiFlags.HasFlag(ANIM.NONINTERRUPT))
        {
            return (false);
        }
        return (true);
    ...
    void SendSoldierPositionEvent(SOLDIERTYPE pSoldier, float dNewXPos, float dNewYPos)
    {
        ...
        GameEvents.AddGameEvent(eJA2Events.S_SETPOSITION, 0, SSetPosition);
    ...
    void SendSoldierDestinationEvent(SOLDIERTYPE pSoldier, int usNewDestination)
    {
        ...
        GameEvents.AddGameEvent(eJA2Events.S_CHANGEDEST, 0, SChangeDest);
    }
    ...
    void SendSoldierSetDirectionEvent(SOLDIERTYPE pSoldier, int usNewDirection)
    {
        ...
        GameEvents.AddGameEvent(eJA2Events.S_SETDIRECTION, 0, SSetDirection);
    }
    ...
    public static void SendSoldierSetDesiredDirectionEvent(SOLDIERTYPE pSoldier, WorldDirections usDesiredDirection)
    {
        ...
        GameEvents.AddGameEvent(eJA2Events.S_SETDESIREDDIRECTION, 0, SSetDesiredDirection);
    }

    void SendGetNewSoldierPathEvent(SOLDIERTYPE pSoldier, int sDestGridNo, AnimationStates usMovementAnim)
    {
        ...
        GameEvents.AddGameEvent(eJA2Events.S_GETNEWPATH, 0, SGetNewPath);
    }
    ...
    public static void SendBeginFireWeaponEvent(SOLDIERTYPE pSoldier, int sTargetGridNo)
    {
        ...
        GameEvents.AddGameEvent(eJA2Events.S_BEGINFIREWEAPON, 0, SBeginFireWeapon);
    }
        ...
        //if ( gTacticalStatus.uiFlags & TURNBASED && (gTacticalStatus.uiFlags & INCOMBAT) )
        {
            ...
            //if ( pSoldier.ubAttackerID != NOBODY )
            {
                ...
                for (cnt = 0; cnt < ubNumToFree; cnt++)
                {
                    ...
                    Overhead.ReduceAttackBusyCount(pSoldier.ubAttackerID, false);
                }
...
//        CHECKV(usAnimSurface != INVALID_ANIMATION_SURFACE);
        ...
        if (bProfileID != NPCID.UNSET)
        {
            ...
            for (iTileCount = 0; iTileCount < pProfileDir.ubNumTiles; iTileCount++)
            {
                ...
                // Check if in bounds
                if (!IsometricUtils.OutOfBounds(pSoldier.sGridNo, sGridNo))
                {
                    if (fRemove)
                    {
                        // Remove from world
    ...
    void EVENT_SoldierBeginBladeAttack(SOLDIERTYPE pSoldier, int sGridNo, WorldDirections ubDirection)
    {
        ...
        else
        {
                ...
                switch (gAnimControl[pTSoldier.usAnimState].ubEndHeight)
                {
                    ...
                    case AnimationHeights.ANIM_PRONE:
                        ...
                        if (gAnimControl[pSoldier.usAnimState].ubEndHeight != AnimationHeights.ANIM_CROUCH)
                        {
                            ...
                            SendChangeSoldierStanceEvent(pSoldier, AnimationHeights.ANIM_CROUCH);
                            pSoldier.usPendingAnimation = AnimationStates.CROUCH_STAB;
                        }
                        else
                        {
                            // USE crouched one
            ...
            else
            {
                ...
                {
                    // Check for corpse!
                    pCorpse = RottingCorpses.GetCorpseAtGridNo(sGridNo, pSoldier.bLevel);
    ...
    void EVENT_SoldierBeginKnifeThrowAttack(SOLDIERTYPE? pSoldier, int sGridNo, WorldDirections ubDirection)
    {
        ...
        EVENT_InitNewSoldierAnim(pSoldier, AnimationStates.THROW_KNIFE, 0, false);
    ...
    void EVENT_SoldierBeginDropBomb(SOLDIERTYPE? pSoldier)
    {
        ...
        switch (gAnimControl[pSoldier.usAnimState].ubHeight)
        {
            case AnimationHeights.ANIM_STAND:
                ...
                EVENT_InitNewSoldierAnim(pSoldier, AnimationStates.PLANT_BOMB, 0, false);
                break;
    ...
    void EVENT_SoldierBeginUseDetonator(SOLDIERTYPE? pSoldier)
    {
        ...
        switch (gAnimControl[pSoldier.usAnimState].ubHeight)
        {
            case AnimationHeights.ANIM_STAND:
                ...
                EVENT_InitNewSoldierAnim(pSoldier, AnimationStates.USE_REMOTE, 0, false);
                break;
    ...
    void EVENT_SoldierBeginFirstAid(SOLDIERTYPE? pSoldier, int sGridNo, WorldDirections ubDirection)
    {
        SOLDIERTYPE? pTSoldier;
        //int uiMercFlags;
        int usSoldierIndex;
        ...
        if (usSoldierIndex != NOBODY)
        {
            ...
            {
                // CHANGE TO ANIMATION
                EVENT_InitNewSoldierAnim(pSoldier, AnimationStates.START_AID, 0, false);
            }
    ...
    int SoldierDressWound(SOLDIERTYPE? pSoldier, SOLDIERTYPE? pVictim, int sKitPts, int sStatus)
    {
        ...
        if (pSoldier.inv[InventorySlot.HANDPOS].usItem == Items.MEDICKIT)
        {
        ...
        if (ubPtsLeft > 0 && pVictim.bLife < (uint)OKLIFE)
        {
...
//        if (ubPtsLeft > 0 && pVictim.bBleeding)
        {
        ...
        Points.DeductPoints(pSoldier, (int)uiUsedAPs, (int)((uiUsedAPs * BP.PER_AP_LT_EFFORT)));
...
//            for (pTSoldier = Menptr, cnt = 0; cnt < MAX_NUM_SOLDIERS; pTSoldier++, cnt++)
            foreach(var pTSoldier in Menptr)
            {
    ...
    public static void HaultSoldierFromSighting(SOLDIERTYPE? pSoldier, bool fFromSightingEnemy)
    {
        // SEND HUALT EVENT!
...
//            AutoPlaceObject(pSoldier, pSoldier.pTempObject, false);
        ...
        else
        {
            ...
            {
                if (pSoldier.ubPendingAction != MERC.NO_PENDING_ACTION && pSoldier.sGridNo == pSoldier.sFinalDestination)
                {
                    pSoldier.ubPendingAction = MERC.NO_PENDING_ACTION;
                }
...
//                TacticalCharacterDialogue(pSoldier, QUOTE.OUT_OF_BREATH);
        ...
        if (pSoldier.bBreath == 0
            && !pSoldier.bCollapsed
            && !(pSoldier.uiStatusFlags.HasFlag(SOLDIER.VEHICLE | SOLDIER.ANIMAL | SOLDIER.MONSTER)))
        {
            // Collapse!
            // OK, Set a flag, because we may still be in the middle of an animation what is not interruptable...
            pSoldier.bBreathCollapsed = 1;
...
//            if (!WorldStructures.OkayToAddStructureToWorld(pSoldier.sGridNo, pSoldier.bLevel, (pStructureFileRef.pDBStructureRef[(int)gOneCDirection[bDirection]]), usOKToAddStructID))
    ...
    public static bool IsValidMovementMode(SOLDIERTYPE pSoldier, AnimationStates usMovementMode)
    {
        // Check, if dest is prone, we can actually do this!
        ...
        if (MercInWater(pSoldier))
        {
            if (usMovementMode == AnimationStates.RUNNING
                || usMovementMode == AnimationStates.SWATTING
                || usMovementMode == AnimationStates.CRAWLING)
            {
                return (false);
            }
        }
    ...
    void SelectMoveAnimationFromStance(SOLDIERTYPE pSoldier)
    {
        ...
        {
            case AnimationHeights.ANIM_STAND:
                EVENT_InitNewSoldierAnim(pSoldier, AnimationStates.WALKING, 0, false);
                break;
            ...
            case AnimationHeights.ANIM_CROUCH:
                EVENT_InitNewSoldierAnim(pSoldier, AnimationStates.SWATTING, 0, false);
                break;
...
//        sOffsetY = (int)pTrav.sOffsetY;
    ...
    public static bool SoldierCarriesTwoHandedWeapon(SOLDIERTYPE pSoldier)
    {
        Items usItem;
    ...
    public static int CheckBleeding(SOLDIERTYPE pSoldier)
    {
        int bBandaged = 0; //,savedOurTurn;
        int iBlood = 0;//NOBLOOD;
...
//                                    TacticalCharacterDialogue(pSoldier, QUOTE.SERIOUSLY_WOUNDED);
    ...
    public static void SoldierBleed(SOLDIERTYPE pSoldier, bool fBandagedBleed)
    {
        int bOldLife;
    ...
    public static void SoldierCollapse(SOLDIERTYPE pSoldier)
    {
        ...
        switch (pSoldier.ubBodyType)
        {
            ...
            case SoldierBodyTypes.QUEENMONSTER:
                ...
                // Give breath back....
                Points.DeductPoints(pSoldier, 0, (int)-5000);
                return;
                break;
        }
...
//        HandleSight(pSoldier, SIGHT.LOOK);
        ...
        if (pSoldier.uiStatusFlags.HasFlag(SOLDIER.ENEMY))
        {
            ...
            if ((gTacticalStatus.bPanicTriggerIsAlarm[0]) == 0 && (gTacticalStatus.ubTheChosenOne == pSoldier.ubID))
            {
...
//        bBandaged = pSoldier.bLifeMax - pSoldier.bLife - pSoldier.bBleeding;
    ...
    public static void HandlePlacingRoofMarker(SOLDIERTYPE pSoldier, int sGridNo, bool fSet, bool fForce)
    {
        LEVELNODE? pRoofNode;
        LEVELNODE? pNode;
...
//                    if (!IndexExistsInRoofLayer(sGridNo, TileIndexes.FIRSTPOINTERS11))
                    {
//                        pNode = AddRoofToTail(sGridNo, TileIndexes.FIRSTPOINTERS11);
...
//                        pNode.ubNaturalShadeLevel = Shading.DEFAULT_SHADE_LEVEL;
                ...
                {
//                    RemoveRoof(sGridNo, TileIndexes.FIRSTPOINTERS11);
                }
            }
        }
    }
...
//        if (bRefuelItem)
        {
            ...
            // CHANGE TO ANIMATION
            EVENT_InitNewSoldierAnim(pSoldier, AnimationStates.REFUEL_VEHICLE, 0, false);
            // SET BUDDY'S ASSIGNMENT TO REPAIR...
        }
    }


    void EVENT_SoldierBeginTakeBlood(SOLDIERTYPE pSoldier, int sGridNo, WorldDirections ubDirection)
    {
        ROTTING_CORPSE? pCorpse;
        ...
        if (pCorpse != null)
        {
            ...
            EVENT_InitNewSoldierAnim(pSoldier, AnimationStates.TAKE_BLOOD_FROM_CORPSE, 0, false);
        }
        else
        {
            // Say NOTHING quote...
            DoMercBattleSound(pSoldier, BATTLE_SOUND.NOTHING);
        }
    }
...
//            return;
    ...
    public static void EVENT_SoldierBeginReloadRobot(SOLDIERTYPE? pSoldier, int sGridNo, WorldDirections ubDirection, InventorySlot ubMercSlot)
    {
        int ubPerson;
...
//            EVENT_InitNewSoldierAnim(pSoldier, RELOAD_ROBOT, 0, false);
    ...
    public static void ChangeToFlybackAnimation(SOLDIERTYPE? pSoldier, WorldDirections bDirection)
    {
        int usNewGridNo;
...
//        EVENT_InitNewSoldierAnim(pSoldier, FLYBACK_HIT, 0, false);
    ...
    public static void ChangeToFallbackAnimation(SOLDIERTYPE pSoldier, WorldDirections bDirection)
    {
        int usNewGridNo;
        ...
        pSoldier.usPathDataSize++;
        pSoldier.sFinalDestination = usNewGridNo;
        EVENT_InternalSetSoldierDestination(pSoldier, pSoldier.usPathingData[pSoldier.usPathIndex], false, AnimationStates.FALLBACK_HIT_STAND);
    ...
    void MercStealFromMerc(SOLDIERTYPE pSoldier, SOLDIERTYPE pTarget)
    {
        ...
        {
            // SEND PENDING ACTION
            pSoldier.ubPendingAction = MERC.STEAL;
            ...
            {
                EVENT_SetSoldierDesiredDirection(pSoldier, ubDirection);
                EVENT_InitNewSoldierAnim(pSoldier, AnimationStates.STEAL_ITEM, 0, false);
            }
...
//            SetUIBusy(pSoldier.ubID);
    ...
    public static void HandleSystemNewAISituation(SOLDIERTYPE? pSoldier, bool fResetABC)
    {
        // Are we an AI guy?
    ...
    public static void InternalPlaySoldierFootstepSound(SOLDIERTYPE? pSoldier)
    {
        int ubRandomSnd;
        int bVolume = 0;//MIDVOLUME;
        // Assume outside
        ...
        if (!(pSoldier.uiStatusFlags.HasFlag(SOLDIER.VEHICLE)))
        {
            ...
            //if ( SoldierOnScreen( pSoldier.ubID ) )
            {
                ...
                else
                {
                    ...
                    else if (pSoldier.bOverTerrainType == TerrainTypeDefines.DIRT_ROAD || pSoldier.bOverTerrainType == TerrainTypeDefines.PAVED_ROAD)
                    {
...
public enum TURNING_FROM_PRONE
{
    OFF = 0,
    ON = 1,
