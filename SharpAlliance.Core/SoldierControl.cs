...
public class SoldierControl
{
    ...
    public static void EVENT_StopMerc(SOLDIERTYPE? pSoldier, int sGridNo, int bDirection)
    {
        int sX, sY;
        ...
        EVENT_SetSoldierPosition(pSoldier, (double)sX, (double)sY);
    ...
    bool ChangeSoldierState(SOLDIERTYPE? pSoldier, int usNewState, int usStartingAniCode, bool fForce)
    {
    ...
    bool ReevaluateEnemyStance(SOLDIERTYPE? pSoldier, int usAnimState)
    {
    ...
    void CheckForFreeupFromHit(SOLDIERTYPE? pSoldier, int uiOldAnimFlags, int uiNewAnimFlags, int usOldAniState, int usNewState)
    {
    ...
    bool EVENT_InitNewSoldierAnim(SOLDIERTYPE? pSoldier, int usNewState, int usStartingAniCode, bool fForce)
    {
        int usNewGridNo = 0;
        int sAPCost = 0;
        int sBPCost = 0;
        ...
        usNewGridNo = NewGridNo((int)pSoldier.sGridNo, (int)DirectionInc(pSoldier.usPathingData[pSoldier.usPathIndex]));
        ...
        if (pSoldier.ubID < MAX_NUM_SOLDIERS)
        {
            ...
            switch (usNewState)
            {
                ...
                case BIGMERC_CROUCH_TRANS_OUTOF:
                    ...
                    {
                        DeductPoints(pSoldier, AP.CROUCH, BP.CROUCH);
                    }
                ...
                case PRONE_DOWN:
                    ...
                    if (pSoldier.fTurningFromPronePosition < TURNING_FROM_PRONE_START_UP_FROM_MOVE && !pSoldier.fDontChargeAPsForStanceChange)
                    {
                        ...
                        {
                            DeductPoints(pSoldier, AP.PRONE, BP.PRONE);
                        }
                    }
                    pSoldier.fDontChargeAPsForStanceChange = false;
                    break;
                ...
                case START_AID:
                    ...
                    DeductPoints(pSoldier, AP.START_FIRST_AID, BP.START_FIRST_AID);
                    break;

                case CUTTING_FENCE:
                    DeductPoints(pSoldier, AP.USEWIRECUTTERS, BP.USEWIRECUTTERS);
                    break;

                case PLANT_BOMB:

                    DeductPoints(pSoldier, AP.DROP_BOMB, 0);
                    break;

                case STEAL_ITEM:

                    DeductPoints(pSoldier, AP.STEAL_ITEM, 0);
                    break;
                ...
                case USE_REMOTE:
                    ...
                    DeductPoints(pSoldier, AP.USE_REMOTE, 0);
                    break;
                ...
                case HOPFENCE:
                    ...
                    DeductPoints(pSoldier, AP.JUMPFENCE, BP.JUMPFENCE);
                    break;
                ...
                case FALLFORWARD_FROMHIT_STAND:
                    ...
                    DeductPoints(pSoldier, AP.FALL_DOWN, BP.FALL_DOWN);
                    break;

                case FALLFORWARD_FROMHIT_CROUCH:

                    DeductPoints(pSoldier, (AP.FALL_DOWN / 2), (BP.FALL_DOWN / 2));
                    break;
                ...
                case CLIMBDOWNROOF:
                    ...
                    DeductPoints(pSoldier, AP.CLIMBOFFROOF, BP.CLIMBOFFROOF);
                    break;
                ...
                case JUMP_OVER_BLOCKING_PERSON:
                    ...
                    // Set path....
                    {
                        ...
                        DeductPoints(pSoldier, AP.JUMP_OVER, BP.JUMP_OVER);

                        usNewGridNo = NewGridNo((int)pSoldier.sGridNo, DirectionInc(pSoldier.bDirection));
                        usNewGridNo = NewGridNo((int)usNewGridNo, DirectionInc(pSoldier.bDirection));
    ...
    void EVENT_InternalSetSoldierPosition(SOLDIERTYPE? pSoldier, float dNewXPos, float dNewYPos, bool fUpdateDest, bool fUpdateFinalDest, bool fForceRemove)
    {
        int sNewGridNo;
    ...
    void EVENT_SetSoldierPositionAndMaybeFinalDest(SOLDIERTYPE? pSoldier, float dNewXPos, float dNewYPos, bool fUpdateFinalDest)
    {
        EVENT_InternalSetSoldierPosition(pSoldier, dNewXPos, dNewYPos, true, fUpdateFinalDest, false);
    }

    void EVENT_SetSoldierPositionAndMaybeFinalDestAndMaybeNotDestination(SOLDIERTYPE? pSoldier, float dNewXPos, float dNewYPos, bool fUpdateDest, bool fUpdateFinalDest)
    {
        EVENT_InternalSetSoldierPosition(pSoldier, dNewXPos, dNewYPos, fUpdateDest, fUpdateFinalDest, false);
    }


    void InternalSetSoldierHeight(SOLDIERTYPE? pSoldier, double dNewHeight, bool fUpdateLevel)
    {
    ...
    void SetSoldierHeight(SOLDIERTYPE? pSoldier, double dNewHeight)
    {
        InternalSetSoldierHeight(pSoldier, dNewHeight, true);
    }


    void SetSoldierGridNo(SOLDIERTYPE? pSoldier, int sNewGridNo, bool fForceRemove)
    {
        bool fInWaterValue;
        int bDir;
        int cnt;
    ...
    void EVENT_FireSoldierWeapon(SOLDIERTYPE? pSoldier, int sTargetGridNo)
    {
        int sTargetXPos, sTargetYPos;
    ...
    int SelectFireAnimation(SOLDIERTYPE? pSoldier, int ubHeight)
    {
        int sDist;
        int usItem;
        double dTargetX;
    ...
    int GetMoveStateBasedOnStance(SOLDIERTYPE? pSoldier, int ubStanceHeight)
    {
        // Determine which animation to do...depending on stance and gun in hand...
    ...
    bool SoldierReadyWeapon(SOLDIERTYPE? pSoldier, int sTargetXPos, int sTargetYPos, bool fEndReady)
    {
        int sFacingDir;

        sFacingDir = GetDirectionFromXY(sTargetXPos, sTargetYPos, pSoldier);

        return (InternalSoldierReadyWeapon(pSoldier, (int)sFacingDir, fEndReady));
    }


    bool InternalSoldierReadyWeapon(SOLDIERTYPE? pSoldier, int sFacingDir, bool fEndReady)
    {
        int usAnimState;
        bool fReturnVal = false;
    ...
    int PickSoldierReadyAnimation(SOLDIERTYPE? pSoldier, bool fEndReady)
    {
    ...
    extern SOLDIERTYPE? FreeUpAttackerGivenTarget(int ubID, int ubTargetID);
    extern SOLDIERTYPE? ReduceAttackBusyGivenTarget(int ubID, int ubTargetID);


    // ATE: THIS FUNCTION IS USED FOR ALL SOLDIER TAKE DAMAGE FUNCTIONS!
    void EVENT_SoldierGotHit(SOLDIERTYPE? pSoldier, int usWeaponIndex, int sDamage, int sBreathLoss, int bDirection, int sRange, int ubAttackerID, int ubSpecial, int ubHitLocation, int sSubsequent, int sLocationGrid)
    {
        int ubCombinedLoss, ubVolume, ubReason;
        ...
        switch (gAnimControl[pSoldier.usAnimState].ubEndHeight)
        {
            case ANIM_STAND:
                ...
                if (ubSpecial == FIRE_WEAPON_BURST_SPECIAL && pSoldier.ubBodyType <= REGFEMALE)
                {
                    ...
                    EVENT_SetSoldierDirection(pSoldier, (int)bDirection);
    ...
    void SoldierGotHitGunFire(SOLDIERTYPE? pSoldier, int usWeaponIndex, int sDamage, int bDirection, int sRange, int ubAttackerID, int ubSpecial, int ubHitLocation)
    {
        int usNewGridNo;
            ...
            if (gAnimControl[pSoldier.usAnimState].ubEndHeight == ANIM_STAND)
            {
                if (ubSpecial == FIRE_WEAPON_HEAD_EXPLODE_SPECIAL)
                {
                    if (gGameSettings.fOptions[TOPTION_BLOOD_N_GORE])
                    {
                        ...
                        {
                            usNewGridNo = NewGridNo((int)pSoldier.sGridNo, (int)(DirectionInc(pSoldier.bDirection)));

                            // CHECK OK DESTINATION!
                            if (OKFallDirection(pSoldier, usNewGridNo, pSoldier.bLevel, pSoldier.bDirection, JFK_HITDEATH))
                            {
                                usNewGridNo = NewGridNo((int)usNewGridNo, (int)(DirectionInc(pSoldier.bDirection)));
                ...
                else if (ubSpecial == FIRE_WEAPON_CHEST_EXPLODE_SPECIAL)
                {
                    if (gGameSettings.fOptions[TOPTION_BLOOD_N_GORE])
                    {
                        if (SpacesAway(pSoldier.sGridNo, Menptr[ubAttackerID].sGridNo) <= MAX_DISTANCE_FOR_MESSY_DEATH)
                        {
                            ...
                            {
                                usNewGridNo = NewGridNo((int)pSoldier.sGridNo, DirectionInc(gOppositeDirection[pSoldier.bDirection]));

                                if (OKFallDirection(pSoldier, usNewGridNo, pSoldier.bLevel, gOppositeDirection[bDirection], FLYBACK_HIT))
                                {
                                    usNewGridNo = NewGridNo((int)usNewGridNo, DirectionInc(gOppositeDirection[bDirection]));
    ...
    void SoldierGotHitExplosion(SOLDIERTYPE? pSoldier, int usWeaponIndex, int sDamage, int bDirection, int sRange, int ubAttackerID, int ubSpecial, int ubHitLocation)
    {
        int sNewGridNo;
        ...
        switch (gAnimControl[pSoldier.usAnimState].ubEndHeight)
        {
            ...
            case ANIM_CROUCH:
                ...
                // Check behind us!
                sNewGridNo = NewGridNo((int)pSoldier.sGridNo, DirectionInc(gOppositeDirection[bDirection]));
    ...
    bool EVENT_InternalGetNewSoldierPath(SOLDIERTYPE? pSoldier, int sDestGridNo, int usMovementAnim, bool fFromUI, bool fForceRestartAnim)
    {
        int iDest;
        int sNewGridNo;
        bool fContinue;
        int uiDist;
        int usAnimState;
        int usMoveAnimState = usMovementAnim;
        int sMercGridNo;
        ...
        if (fContinue)
        {
            ...
            sNewGridNo = NewGridNo((int)pSoldier.sGridNo, DirectionInc((int)pSoldier.usPathingData[pSoldier.usPathIndex]));
    ...
    void EVENT_GetNewSoldierPath(SOLDIERTYPE? pSoldier, int sDestGridNo, int usMovementAnim)
    {
    ...
    void ChangeSoldierStance(SOLDIERTYPE? pSoldier, int ubDesiredStance)
    {
        int usNewState;
    ...
    void EVENT_InternalSetSoldierDestination(SOLDIERTYPE? pSoldier, int usNewDirection, bool fFromMove, int usAnimState)
    {
        int usNewGridNo;
        int sXPos, sYPos;

        // Get dest gridno, convert to center coords
        usNewGridNo = NewGridNo((int)pSoldier.sGridNo, DirectionInc(usNewDirection));
    ...
    int MultiTiledTurnDirection(SOLDIERTYPE? pSoldier, int bStartDirection, int bDesiredDirection)
    {
        int bTurningIncrement;
        int bCurrentDirection;
        int bLoop;
        int usStructureID, usAnimSurface;
        STRUCTURE_FILE_REF* pStructureFileRef;
        bool fOk = false;

        // start by trying to turn in quickest direction
        bTurningIncrement = (int)QuickestDirection(bStartDirection, bDesiredDirection);
    ...
    void EVENT_InternalSetSoldierDesiredDirection(SOLDIERTYPE? pSoldier, int usNewDirection, bool fInitalMove, int usAnimState)
    {
        ...
        pSoldier.bDesiredDirection = (int)usNewDirection;
        ...
        {
            if (gAnimControl[usAnimState].uiFlags & (ANIM_BREATH | ANIM_OK_CHARGE_AP.FOR_TURN | ANIM_FIREREADY) && !fInitalMove && !pSoldier.fDontChargeTurningAPs)
            {
                ...
                switch (gAnimControl[usAnimState].ubEndHeight)
                {
                    // Now change to appropriate animation
                    case ANIM_STAND:
                        DeductPoints(pSoldier, AP.LOOK_STANDING, 0);
                        break;

                    case ANIM_CROUCH:
                        DeductPoints(pSoldier, AP.LOOK_CROUCHED, 0);
                        break;

                    case ANIM_PRONE:
                        DeductPoints(pSoldier, AP.LOOK_PRONE, 0);
                        break;
                }
        ...
        else
        {
            ...
            {
                pSoldier.bTurningIncrement = (int)MultiTiledTurnDirection(pSoldier, pSoldier.bDirection, pSoldier.bDesiredDirection);
            }
            else
            {
    ...
    void EVENT_SetSoldierDesiredDirection(SOLDIERTYPE? pSoldier, int usNewDirection)
    {
        EVENT_InternalSetSoldierDesiredDirection(pSoldier, usNewDirection, false, pSoldier.usAnimState);
    }


    void EVENT_SetSoldierDirection(SOLDIERTYPE? pSoldier, int usNewDirection)
    {
        // Remove old location data
        HandleAnimationProfile(pSoldier, pSoldier.usAnimState, true);

        pSoldier.bDirection = (int)usNewDirection;
    ...
    int[] gDirectionFrom8to2 = { 0, 0, 1, 1, 0, 1, 1, 0 };


    bool ConvertAniCodeToAniFrame(SOLDIERTYPE? pSoldier, int usAniFrame)
    {
        int usAnimSurface;
        int ubTempDir;
...
int	gOrangeGlowG[]=
{
    ...
    bool CreateSoldierPalettes(SOLDIERTYPE? pSoldier)
    {
    ...
    void CalculateSoldierAniSpeed(SOLDIERTYPE? pSoldier, SOLDIERTYPE? pStatsSoldier)
    {
        ...
        if (!(gTacticalStatus.uiFlags & INCOMBAT))
        {
            ...
            {
                pSoldier.sAniDelay = (int)(pSoldier.sAniDelay * 2);
            }
    ...
    bool LoadPaletteData()
    {
        ...
        {
            if (!FileRead(hFile, &gubpNumReplacementsPerRange[cnt], sizeof(int), null))
            {
                return (false);
            }
        }
        ...
        for (cnt = 0; cnt < guiNumPaletteSubRanges; cnt++)
        {
            if (!FileRead(hFile, &gpPaletteSubRanges[cnt].ubStart, sizeof(int), null))
            {
                return (false);
            }
            if (!FileRead(hFile, &gpPaletteSubRanges[cnt].ubEnd, sizeof(int), null))
            {
                return (false);
            }
        }
        ...
        {
            if (!FileRead(hFile, &gpPalRep[cnt].r[cnt2], sizeof(int), null))
            {
                return (false);
            }
            if (!FileRead(hFile, &gpPalRep[cnt].g[cnt2], sizeof(int), null))
            {
                return (false);
            }
            if (!FileRead(hFile, &gpPalRep[cnt].b[cnt2], sizeof(int), null))
            {
                return (false);
            }
        }
...
int GetNewSoldierStateFromNewStance(SOLDIERTYPE? pSoldier, int ubDesiredStance)
{
    int usNewState;
    int bCurrentHeight;
...
void BeginSoldierClimbUpRoof(SOLDIERTYPE? pSoldier)
{
    ...
    if (FindHeigherLevel(pSoldier, pSoldier.sGridNo, pSoldier.bDirection, &bNewDirection) && (pSoldier.bLevel == 0))
    {
        if (EnoughPoints(pSoldier, GetAPsToClimbRoof(pSoldier, false), 0, true))
        {
            ...
            pSoldier.sTempNewGridNo = NewGridNo((int)pSoldier.sGridNo, (int)DirectionInc(bNewDirection));
...
void BeginSoldierClimbFence(SOLDIERTYPE? pSoldier)
{
    ...
    {
        pSoldier.sTempNewGridNo = NewGridNo((int)pSoldier.sGridNo, (int)DirectionInc(bDirection));
...
void BeginSoldierGetup(SOLDIERTYPE? pSoldier)
{
    ...
    else if (pSoldier.bSleepDrugCounter > 0)
    {
        ...
        {
            // succumb to the drug!
            DeductPoints(pSoldier, 0, (int)(pSoldier.bBreathMax * 100));
            SoldierCollapse(pSoldier);
        }
    }
...
void HandleTakeDamageDeath(SOLDIERTYPE? pSoldier, int bOldLife, int ubReason)
{
    switch (ubReason)
    {
...
int SoldierTakeDamage(SOLDIERTYPE? pSoldier, int bHeight, int sLifeDeduct, int sBreathLoss, int ubReason, int ubAttacker, int sSourceGrid, int sSubsequent, bool fShowDamage)
{
    int bOldLife;
    int ubCombinedLoss;
    int bBandage;
    ...
    switch (pSoldier.bTeam)
    {
        case ENEMY_TEAM:
            ...
            if (StrategicMap[CALCULATE_STRATEGIC_INDEX(gWorldSectorX, gWorldSectorY)].fEnemyControlled || SectorInfo[SECTOR(gWorldSectorX, gWorldSectorY)].ubTraversability[THROUGH_STRATEGIC_MOVE] != TOWN)
            {
                ...
                UpdateLastDayOfPlayerActivity((int)GetWorldDay());
            }
            break;
        case CREATURE_TEAM:
...
extern bool IsMercSayingDialogue(int ubProfileID);


bool InternalDoMercBattleSound(SOLDIERTYPE? pSoldier, int ubBattleSoundID, int bSpecialCode)
{
...
bool CheckSoldierHitRoof(SOLDIERTYPE? pSoldier)
{
    ...
    if (FindLowerLevel(pSoldier, pSoldier.sGridNo, pSoldier.bDirection, &bNewDirection) && (pSoldier.bLevel > 0))
    {
        ...
        if (gAnimControl[pSoldier.usAnimState].ubHeight == ANIM_STAND)
        {
            ...
            sNewGridNo = NewGridNo((int)pSoldier.sGridNo, DirectionInc(gOppositeDirection[bNewDirection]));
            if (!NewOKDestination(pSoldier, sNewGridNo, true, 0))
            {
                return (false);
            }
            sNewGridNo = NewGridNo((int)sNewGridNo, DirectionInc(gOppositeDirection[bNewDirection]));
            if (!NewOKDestination(pSoldier, sNewGridNo, true, 0))
            {
                return (false);
            }
            ...
            if (fDoForwards)
            {
                pSoldier.sTempNewGridNo = NewGridNo((int)pSoldier.sGridNo, (int)(-1 * DirectionInc(bNewDirection)));
                pSoldier.sTempNewGridNo = NewGridNo((int)pSoldier.sTempNewGridNo, (int)(-1 * DirectionInc(bNewDirection)));
...
void MoveMerc(SOLDIERTYPE? pSoldier, double dMovementChange, double dAngle, bool fCheckRange)
{
    int dDegAngle;
    double dDeltaPos;
    double dXPos, dYPos;
    ...
    dDeltaPos = (double)(dMovementChange * sin(dAngle));
    ...
    dDeltaPos = (double)(dMovementChange * cos(dAngle));
...
int GetDirectionFromGridNo(int sGridNo, SOLDIERTYPE? pSoldier)
{
    int sXPos, sYPos;
...
int GetDirectionFromXY(int sXPos, int sYPos, SOLDIERTYPE? pSoldier)
{
    int sXPos2, sYPos2;
...
//#if 0
int atan8(int sXPos, int sYPos, int sXPos2, int sYPos2)
{
...
int atan8FromAngle(DOUBLE angle)
{
    int mFacing = WEST;
...
void SendSoldierSetDesiredDirectionEvent(SOLDIERTYPE? pSoldier, int usDesiredDirection)
{
    // Sent event for position update
...
void SendChangeSoldierStanceEvent(SOLDIERTYPE? pSoldier, int ubNewStance)
{
#if 0
...
void HandleAnimationProfile(SOLDIERTYPE? pSoldier, int usAnimState, bool fRemove)
{
    //#if 0
    ANIM_PROF* pProfile;
...
void EVENT_SoldierBeginBladeAttack(SOLDIERTYPE? pSoldier, int sGridNo, int ubDirection)
{
    SOLDIERTYPE? pTSoldier;
    //int uiMercFlags;
    int usSoldierIndex;
...
void EVENT_SoldierBeginPunchAttack(SOLDIERTYPE? pSoldier, int sGridNo, int ubDirection)
{
...
void InternalReceivingSoldierCancelServices(SOLDIERTYPE? pSoldier, bool fPlayEndAnim)
{
    SOLDIERTYPE? pTSoldier;
    int cnt;
...
void InternalGivingSoldierCancelServices(SOLDIERTYPE? pSoldier, bool fPlayEndAnim)
{
    SOLDIERTYPE? pTSoldier;
...
// HUALT EVENT IS USED TO STOP A MERC - NETWORKING SHOULD CHECK / ADJUST TO GRIDNO?
void EVENT_StopMerc(SOLDIERTYPE? pSoldier, int sGridNo, int bDirection)
{
    int sX, sY;
...
void ReLoadSoldierAnimationDueToHandItemChange(SOLDIERTYPE? pSoldier, int usOldItem, int usNewItem)
{
...
int* CreateEnemyGlow16BPPPalette(SGPPaletteEntry* pPalette, int rscale, int gscale, bool fAdjustGreen)
{
    ...
    for (cnt = 0; cnt < 256; cnt++)
    {
        ...
        r = (int)__min(rmod, 255);
        g = (int)__min(gmod, 255);
        b = (int)__min(bmod, 255);
...
int* CreateEnemyGreyGlow16BPPPalette(SGPPaletteEntry* pPalette, int rscale, int gscale, bool fAdjustGreen)
{
    int* p16BPPPalette, r16, g16, b16, usColor;
    int cnt, lumin;
    int rmod, gmod, bmod;
...
bool CheckForBreathCollapse(SOLDIERTYPE? pSoldier)
{
...
bool InternalIsValidStance(SOLDIERTYPE? pSoldier, int bDirection, int bNewStance)
{
...
void GetActualSoldierAnimOffsets(SOLDIERTYPE? pSoldier, int* sOffsetX, int* sOffsetY)
{
    int usAnimSurface;
    ETRLEObject* pTrav;
    ...
    if (pSoldier.bLife != 0)
    {
        ...
        if ((pSoldier.bBleeding > MIN_BLEEDING_THRESHOLD) || pSoldier.bLife < OKLIFE)
        {
            ...
            if ((pSoldier.ubServiceCount == 0) && (AnyDoctorWhoCanHealThisPatient(pSoldier, HEALABLE_EVER) == null))
            {
                ...
                if (!(gTacticalStatus.uiFlags & TURNBASED) || !(gTacticalStatus.uiFlags & INCOMBAT))
                {
                    pSoldier.dNextBleed -= (double)RT_NEXT_BLEED_MODIFIER;
                }
                else
                {
...
double CalcSoldierNextBleed(SOLDIERTYPE? pSoldier)
{
    ...
    return ((double)1 + (double)((pSoldier.bLife + bBandaged / 2) / (10 + pSoldier.bTilesMoved)));  // min = 1
}

double CalcSoldierNextUnmovingBleed(SOLDIERTYPE? pSoldier)
{
    int bBandaged;
    ...
    return ((double)1 + (double)((pSoldier.bLife + bBandaged / 2) / 10));  // min = 1
}
...
void PickPickupAnimation(SOLDIERTYPE? pSoldier, int iItemIndex, int sGridNo, int bZLevel)
{
    int bDirection;
    ...
    if (sGridNo != pSoldier.sGridNo)
    {
        ...
        bDirection = (int)GetDirectionFromGridNo(sGridNo, pSoldier);
            ...
            switch (gAnimControl[pSoldier.usAnimState].ubHeight)
            {
                case ANIM_STAND:
                    ...
                    if (bZLevel > 0)
                    {
                        //#if 0
                        // Get direction to face....
                        if ((pStructure = FindStructure((int)sGridNo, (STRUCTURE_HASITEMONTOP | STRUCTURE_OPENABLE))) != null)
                        {
...
void EVENT_SoldierBeginRepair(SOLDIERTYPE? pSoldier, int sGridNo, int ubDirection)
{
    int bRepairItem;
    int ubID;
...
void EVENT_SoldierBeginRefuel(SOLDIERTYPE? pSoldier, int sGridNo, int ubDirection)
{
    int bRefuelItem;
    int ubID;
...
void EVENT_SoldierBeginAttachCan(SOLDIERTYPE? pSoldier, int sGridNo, int ubDirection)
{
    STRUCTURE* pStructure;
...
void SetSoldierCowerState(SOLDIERTYPE? pSoldier, bool fOn)
{
    // Robot's don't cower!
    if (pSoldier.ubBodyType == ROBOTNOWEAPON)
    {
...
{
    int sFacingDir, sXPos, sYPos, sAPCost;
    SOLDIERTYPE? pTSoldier;
    int uiRange;
...
bool IsValidSecondHandShot(SOLDIERTYPE? pSoldier)
{
